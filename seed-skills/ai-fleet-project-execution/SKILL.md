...
2. `printf 'header = "Authorization: Bearer %s"\n' "$(cat store/.dashboard-token)" | curl -s -K - -X POST http://localhost:3420/api/agents/<name>/start` → `{"ok":true}` válasz (a token curl-configon megy, nem argv-ben: a `/proc/<pid>/cmdline` bárki számára olvasható)
