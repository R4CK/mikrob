...
2. `printf 'Authorization: Bearer %s\n' "$(cat store/.dashboard-token)" | curl -s -H @- -X POST http://localhost:3420/api/agents/<name>/start` → `{"ok":true}` válasz (a token stdin-rol jon a `-H @-`-en, nem argv-ben: a `/proc/<pid>/cmdline` barki szamara olvashato)
