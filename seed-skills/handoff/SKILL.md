...
# The dashboard token is read from STDIN (`-H @-`), NEVER built into an argv header:
...
AUTH | curl -s -H @- "http://localhost:$PORT/api/kanban" | AGENT_ID="$AGENT_ID" python3 -c "
import json,os,sys
...
AUTH | curl -s -H @- "http://localhost:$PORT/api/memories?agent=$AGENT_ID&category=hot&limit=10"

# Recent warm memories (project context)
AUTH | curl -s -H @- "http://localhost:$PORT/api/memories?agent=$AGENT_ID&category=warm&limit=5"

# Today's daily log
DATE=$(date +%Y-%m-%d)
AUTH | curl -s -H @- "http://localhost:$PORT/api/daily-log?agent=$AGENT_ID&date=$DATE"
```
...
| curl -s -H @- -X POST http://localhost:$PORT/api/messages \
