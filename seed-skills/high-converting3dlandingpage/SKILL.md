...
## When to use
Use whenever Peti asks for a landing page whose job is to convert: product launches, SaaS sign-ups, waitlists, sales/marketing pages, or any "make it wow AND make it convert" request. Also use when Peti mentions a hero scene, animated product demo, immersive/interactive landing, WebGL/Three.js/R3F landing, CTA optimization, or an awwwards-style page that must still sell. Do NOT use for pure content sites, dashboards, or internal tools — for those, prefer the general 3D or UI skills.

## Instructions
1. **Clarify the conversion goal first.** Ask Peti (or state an assumption) for the ONE primary action (sign up, buy, book a demo), the target audience, and the core value proposition. Everything else serves this.
...
10. **Close with a build-ready spec** Peti or a frontend agent can implement directly.
...
**Example 1**
Input (Peti): "Kell egy 3D landing egy AI jegyzetelő appnak, minél többen regisztráljanak."
...
**Example 2**
Input (Peti): "Make it convert — landing for a premium ergonomic chair."
...
## Language rules
- Talk to Peti in **Hungarian** (natural, conversational). Refer to the user only as **Peti**.
...
- Section headings in deliverables may stay English for build-readiness; explanations to Peti in Hungarian.
