...
5. **"Fails open."** On error or missing field, the code defaults to allow (e.g. `[undefined]` role that some check happens to pass). Security must fail CLOSED. Two mechanical forms that hide well: (a) a **reason-string protocol** where `reason == ""` means ALLOW — any silent skip inside the checker becomes an allow; prefer "empty verdict means REFUSE". (b) In shell, `[ "$a" -gt "$b" ]` with a NON-NUMERIC operand prints an integer error to **stderr** and evaluates **FALSE** — so a "if over the limit, refuse" guard silently allows. Whenever a threshold comes from the environment/config, validate it before comparing (`case "$X" in ""|*[!0-9]*) X=<default> ;; esac`). An empty value often happens to be safe (`${X:-50}` supplies the default) while garbage is not, so the failure leans the wrong way. Mutation testing does NOT catch this: mutating the call sites and the fallback direction leaves missing *config validation* invisible.
...
12. **"The fix widened one axis and narrowed another."** When you gate a fix that HARDENS a detection rule (regex/filter/allowlist/guard), the reported class closing is only half the verdict — the other half is whether the new rule still catches everything the OLD one caught. Widening usually costs something elsewhere: a real case dropped a regex's case-insensitive flag to make an uppercase-token alternative discriminating, closed 7 attack shapes, and silently reopened 3 the previous version already caught (`CURL …`, `… HTTPS://…`). Method: put OLD and NEW side by side in one probe over the same corpus and print both columns per sample (`OLD-HIT/old-miss  NEW-HIT/new-MISS`). **Any `OLD-HIT -> new-MISS` row is a regression and blocks the gate**, however much the fix improved elsewhere. Fix shape: narrow the change to what actually needs it (case-fold only the literals) instead of restoring the blanket flag. Watch for a threat-model error in the justification — "the shell only ever writes it lower-case" is about OUR output, while the filter reads ATTACKER-authored text. Same discipline after a ROLLBACK: the running system moved backwards past merged fixes, so re-run the fix-class greps against the live HEAD and the pre-rollback tip and report the per-class counts; a `done` card is evidence about a commit, never about what is deployed.
...
- **A control can be inert for two independent reasons: not configured, OR configured but never delivered. Reading the code shows you only the first.** When a card introduces a NEW `env.X` read, the logic can be flawless while the value never reaches the process -- and an OPT-IN control that treats absence as "skip the check" then fails OPEN with no boot error and no log line, so the operator believes it is protecting them. Measured (card e792cfea, my own GO, 2026-09-17): `SUPERADMIN_WEB_ORIGIN` was read in `main.ts` and documented in `.env.prod.example`, but was absent from the `environment:` allowlist in `infra/docker-compose.app.yml` (whose api service deliberately has no `env_file` blanket include), so it could never arrive. I GO'd the validator logic and even advised operations to set it -- which would have produced a host binding that silently did nothing. The guard test `apps/api/src/api-env-allowlist.test.ts` already existed and names exactly this ("these are read by the api but not passed to its container"); I simply did not run it. Third recurrence in the same file (`SUPERADMIN_MAIL_FROM`, then `MAIL_FROM` -- the compose comments say "Same allowlist lesson as SUPERADMIN_MAIL_FROM above"), i.e. the guard exists but was never wired into the GATE procedure. So: grep the diff for new env reads FIRST, then run the deployment-allowlist guard, then ask the fail-direction question below.
...
- Does the diff introduce a NEW environment variable (`env.X`, `process.env.X`, a new config key)? If yes, BOTH of these are required before a GO, and neither is satisfied by reading the code:
  1. **Delivery.** Run the repo's deployment-allowlist guard (in CleanCore: `apps/api/src/api-env-allowlist.test.ts`) and confirm the variable is actually passed to the running process -- compose `environment:` list, secret manager, or whatever the deployment uses. Documented in a `.env.*.example` is NOT delivery.
  2. **Fail direction.** If the value does NOT arrive, does the control fail CLOSED (refuse to boot / deny) or OPEN (skip the check)? Fail-open on absence is itself a finding worth reporting even when today's wiring is correct, because wiring can silently regress later while the code keeps looking right.
