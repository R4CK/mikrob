...
1d. **Egy in_progress/dispatchelt Fázis/Feladat subtaskjai (a predecessor-lánc mentén) élvezzenek elsőbbséget, még akkor is, ha a subtask MÁS ügynökhöz van kiosztva, mint aki a szülőn dolgozik (Peti szabály 2026-09-07, Telegram).** Amikor egy már folyamatban lévő Fázis/Feladat egyik gyerek-kártyája (predecessor) egy adott ügynökhöz van kiosztva, annak az ügynöknek a saját dispatch-sorában ez a kártya elsőbbséget élvez a nominálisan magasabb prioritású, de a szülőhöz nem kapcsolódó planned kártyákkal szemben is -- a cél, hogy a TELJES Fázis/Feladat egyszerre tudjon záródni, ne várakozzon egy már elkezdett munka egy el nem indított predecessor miatt. Ez pontosan azért működik mechanikusan, mert az 1c szabály szerint a valódi sorrend-függőség EXPLICIT predecessor/successor élként van bedrótozva (nem próza): a dispatch-döntésnél (6./11a. szabály, heartbeat C szekció 4. lépés) nem elég az egyéni kártya `priority` mezője, meg kell nézni, blokkol-e a kártya egy már folyamatban lévő testvér/szülő láncot -- ha igen, azt kell előrébb venni a soros dispatchben, függetlenül attól, kihez van kiosztva. Gyakorlati eset (dc35fa1a-epic-en kívüli élő példa, 2026-09-07): 6c711bce (backend3) nem indítható, mert predecessor-e, 2cc9435d, még planned és senkinek nincs aktívan dolgozva -- ilyenkor MikroB-nak (vagy a self-advance-nak) 2cc9435d-t kell előreveznie, nem hagyni, hogy a lánc csendben álljon.

1e. **Az új-fejlesztés-stop alatt is AZONNAL indulnak az elkezdett munka kapcsolt kártyái (Peti szabály 2026-09-26, Telegram 9472).** Ha a heti kvóta miatt új fejlesztés nem indulhat (`newDevStopActive`), a már elkezdett (in_progress) Fázisok/kártyák gyerek-kártyái, predecessorai és successorai ettől függetlenül futhatnak, és a tervezett ilyen kártyák AZONNAL induljanak -- a stop csak a független, új munkát fékezi. Egy elkezdett Fázis, aminek a gyerekei nem indulnak, nem tud záródni, és többet éget (kontextus, újraellenőrzés), mint amennyit a stop megtakarít. Végrehajtás: a tábla API a stop alatt az ügynök saját `PUT in_progress`-ét 409-cel utasítja el, ezért MikroB nyitja meg (`force:true`, `actor:"mikrob"`, `reason` a szabályra hivatkozva) ügynökönként a sor következő kapcsolt kártyáját, amint az előző REVIEW+waiting-be került -- egyszerre egy kártya ügynökönként, az ügynök-plafonon (agent-cap) belül. Idle, gate-munka nélküli gate-ügynök parkolható, hogy a slot egy kapcsolt kártya építőjéé legyen.
...
4. **Készterméket csak NEM a készítő ellenőrizhet -- KOCKÁZAT-ALAPÚ gate-tiering (Peti szabály 2026-07-05).** Minden kész kártyát MINIMUM 2 független ügynök ellenőriz; a készítő SOHA nem ellenőrzi a sajátját. Tesztelési/sign-off jogköre KIZÁRÓLAG a 3-tagú gate-poolnak: **`qa-engineer`** (funkcionális: teszt-piramis, regresszió, acceptance), **`cybersecurity-redteam`** (Cybersec, per-finding: STRIDE, OWASP, bypass, exploit + fix) és **`cybered`** (Cybered, adverzariális: assume-breach, teljes kill-chain, legális aktív védelem -- KIZÁRÓLAG engedélyezett hatókörön). **MikroB TTE-feladata (állandó orchestrátor-kötelesség): kártyánként kiválasztani/váltogatni a gate-tagokat a kockázat szerint:**
   ...
   **4a. MINDEN gate-verdiktre AZONNAL reagálni -- a flotta SOHA ne várjon MikroB-ra (Peti szabály 2026-07-12, 3x beragadás után).** A `waiting` kártya egy gate-verdikttel MikroB akció-trigger, nem "kész". MINDEN MikroB-ébredéskor (Peti-üzenet, scheduled-task, orchestrator-tick -- BÁRMI), a válasz/új-dispatch ELŐTT KÖTELEZŐ egy board-reconciliation sweep a `waiting` kártyákon: (a) minden kijelölt gate PASS/GO + nincs kötött-blokk -> AZONNAL `done` + zárás (majd szülő-fázis auto-close, 5. szabály). **A "minden gate PASS/GO" NEM elég önmagában (kártya 1c4f9af1, Cybered lelete): ugyanarra a SHÁRA is kell szólniuk.** Delta-gate-elésnél (NO-GO utáni újraellenőrzés) előfordul, hogy az egyik gate már a javított shára ad verdiktet, a másik még a javítás előttire szólót hordozza -- felszínesen minden zöld, valójában nem ugyanazt a kódot nézték meg. Zárás előtt: `printf 'Authorization: Bearer %s\n' "$(cat /home/neon/marveen/store/.dashboard-token)" | curl -H @- -s http://localhost:3420/api/kanban/<id>/comments | python3 /home/neon/marveen/store/gate-closure-check.py [qa,cybersec]` -- egyetlen sort ír ki: `AGREE|<sha>` (zárható), `DISAGREE` (a fenti csapda, ne zárd), `FAILED`, `MISSING`, vagy `NOSHA` (a verdikt nem adott shát, tehát az egyezés nem ellenőrizhető -- ez a verdiktek 8%-a, emberi döntés). **Ha a kártya munkáját ÚJRAÉPÍTETTÉK (rebuild/rebase/cherry-pick), add hozzá a `--expect <a kártya MOSTANI shája>` kapcsolót** -- e nélkül az eszköz csak azt kérdezi, hogy a gate-ek EGYMÁSSAL egyeznek-e, és két RÉGI verdikt tökéletesen egyezik egymással, tehát `AGREE`-t kapsz olyan kódra, ami már nincs sehol (élesben mérve a c458ba0e/acab6155/f8b52ff2 hármason: `AGREE|6bb97eba`, miközben a szállítmány már 80de05f5 volt). `--expect`-tel ez `STALE|<verdikt-sha>|<elvárt-sha>` -> NE zárd, delta-gate kell. A `--expect` nem írja felül a `FAILED`/`MISSING`/`NOSHA` válaszokat. **A `--expect` 2026-09-05-től ALAPÉRTELMEZETT (kártya 2003e04b): ha nem adod meg, az eszköz a kártya LEGFRISSEBB `REVIEW` kommentjének `Gate-SHA:` sorát használja elvárt shaként** -- pont az a hívó volt védtelen, aki elfelejtette kitenni a kapcsolót (Cybered ezt élesben demonstrálta az e80c011a-n, perceken belül a kapcsoló landolása után). Az összevetés NEM sha-egyenlőség: egy eltérő sha előbb TARTALOM szerint dől el (a két commit ugyanazt adja-e a kártya által szállított fájlokra), mert a flotta 557 AGREE kártyájából 38-nál tér el a REVIEW shája a gate-ekétől, és ebből 23 ártalmatlan -- tipikusan a munka-commit kontra az őt landoló merge, ahol csak a `package.json` verziója, a `DECISIONS.md` vagy a `README.md` mozdult. Puszta sha-egyenlőséggel a zárások 6,8%-ára szólt volna hamis riasztás, ami pár nap alatt megtanítja mindenkinek, hogy hagyja figyelmen kívül. Új válasz: `UNRESOLVED|<verdikt-sha>|<elvárt-sha>` -- a shák eltérnek ÉS az eltérés nem ítélhető meg (nincs meg a klón, törölt commit): ez NEM zárható, mert a "nem tudom ellenőrizni" nem azonos az "ellenőriztem, rendben"-nel. Új válasz (kártya c52e2823, 2026-09-06): `UNSUPERSEDED|<gate-ek>|<ki mondta>` -- áll egy elutasítás, ami NEM az adott gate szerepét viselő szerzőtől jött (tipikusan a rutinszerű `<GATE> <VERDIKT> ELFOGADVA` nyugtázás), vagy egy testvér-gate-től sha nélkül, és nincs mivel bizonyítani, hogy egy későbbi PASS felülírta-e. Ez sem nem `AGREE` (nem zárható), sem nem `FAILED` (nem dobható vissza automatikusan): emberi/MikroB-döntés. A gate-ek és a beszélők pozíció szerint párosodnak. Ugyanezen a kártyán szűnt meg az ellenkező irányú hiba is: egy ilyen nyugtázás, ami IDŐBEN a gate saját, későbbi PASS-a ELŐTT áll, többé nem blokkol (élesben mérve: a tábla 348 kommentelt kártyájából 5 válasza változik, mind ellenőrizve egyenként). A régi, kapcsoló nélküli viselkedés a `--no-expect`-tel áll vissza. A gates-argumentum elhagyható; ekkor a jelen lévő verdiktekből következtet, és ezt ki is írja -- egy KIMONDATLAN gate-kijelölés pont így marad észrevétlen; (b) bármely gate FAIL/NO-GO -> AZONNAL `in_progress` + re-dispatch a felelősnek a bug-jelentéssel (parkolt ügynököt előbb `start`); (c) REVIEW van, de gate még nincs dispatchelve -> dispatcheld a kijelölt gate-eket; (d) kötött-blokk (pl. Cybered WC1/WC2, Peti-infra) -> maradjon `waiting`, EGYSZER annotálva. Egy verdikt-után-veszteglő kártya = megállt flotta és idle-de-futó gate-ügynökök (kvótaégés). Ezt automatizálja a `gate-reconciler` scheduled-task 5 percenként; de a reflex akkor is kötelező, ha nem a task ébresztett. Kapcsolódó tanulság: [[close-gate-passed-cards-immediately]].
   ...
   **4d. Egy suite-/teszt-SZÁM egy verdiktben/REVIEW-ban MONDJA MEG, MELYIK FÁN/SHÁN mérték (Cybersec javaslata, 2026-09-13).** Ha egy verdikt vagy REVIEW konkrét számot idéz egy teszt-futásból (pl. "786/786 zöld", "151/151 teszt"), a szám mellett/alatt szerepeljen, MELYIK fán/shán futott (pl. "a `fleet-test.sh --ref <sha>` szerint" vagy "a merge EREDMÉNYÉN, nem az előtte lévő ágon"). MIÉRT: a `Gate-SHA:` sor (4b.) ezt már kikényszeríti a KÓDRA (melyik commitot néztük), de a MÉRÉSRE nem -- egy szám az őt szolgáltató fa megnevezése nélkül nem bizonyíték, csak szám. Cybersec ugyanazon a napon négy esetben csúszott bele (vagy majdnem bele) ugyanebbe a gyökér-hibába: egy `| tail -30`-lal levágott pipeline-státusz (exit 0 conflicttel), egy suite-log ami a saját önteszt-nevét nem írta ki (report-viselkedés, nem hiányzó lefedettség), egy csendben-nem-alkalmazódott mutáció ami "6 passed"-ként olvasódott VAKUUM pinnek, és a legsúlyosabb: a `fleet-test.sh` kimenetének ELSŐ sora egy MERGE ELŐTTI developot nevezett meg, a végén álló "786/786 zöld" szám mégis bekerült egy REVIEW-ba, mintha a merge eredményét mérte volna -- ez a landolást pirosra vitte. Lásd [[read-the-head-of-the-output-not-just-the-tail]]. Nem igényel új eszközt/hookot: egy mondat a REVIEW/verdikt-sablonban.
...
11. **SELF-ADVANCE -- a flotta ÖNJÁRÓ, sosem áll MikroB-ra várva (Peti szabály 2026-07-12, a 6. szabály végrehajtási mechanizmusa).** Minden flotta-ügynök, AMINT befejez egy kártyát, AZONNAL maga veszi a következő munkáját, NEM vár MikroB dispatchre: **(a) Mérnöki ügynök** (backend/fullstack/fron-ted/fron-teddy/...): ELŐSZÖR a "REVIEW" komment, CSAK UTÁNA a kártya `waiting`-re állítása (kártya e98a34d3, SIGSTOP-orphan elkerülése -- lásd 4. szabály) + rövid trusted-peer jelzés MikroB-nak (a gate-hez), majd `curl` a kanbanra -> **ELŐSZÖR** `cat store/project-dispatch-priority.json`: ha a `priority` tömb NEM üres, a benne felsorolt projekt(ek) kártyái KIZÁRÓLAGOSAK -- amíg ezen projekt(ek)ben van akár EGY dispatchelhető (nem blokkolt) `planned` kártya, TILOS más projektből kártyát felvenni, függetlenül a prioritási szinttől (egy MikroB-projektbeli `low` is megelőz egy másik projektbeli `urgent`-et). Csak akkor vált vissza a normál, projekt-független válogatásra, ha a KORÁBBAN KIJELÖLT projekthez tartozó `planned` oszlop TELJESEN kiürül (Peti szabály 2026-09-07, Telegram, pontosított megfogalmazás). Üres/hiányzó fájl esetén nincs megszorítás, a régi viselkedés áll. Ez a kizárólagosság csak az ÚJ kártyafelvételre vonatkozik, a már folyamatban lévő munkát nem szakítja meg. Ezután a kiválasztott projekt(ek)en belül a legmagasabb prioritású (urgent>high>normal>low) `planned` kártya, aminek az assignee-je ő (vagy a `@<neve>` label rajta van) és NINCS `BLOKKOLT`/infra-blokk -> **a felvétel `store/self-advance-pickup.sh <ügynök> <cardId>`-n át megy** (kártya 3906d77b, NEM kézzel curl-özött `PUT in_progress`): a script Ollama-egészség-ellenőrzés után (sose blokkol, hiányzó/lefagyott Ollama esetén csendben tovább ONLINE-ra esik) lefuttatja a `card-build-route.sh`-t `CARD_BUILD_ROUTE_DISPATCHER=self-advance` attribúcióval, LOCAL verdiktnél `offload-dispatch.sh`-val helyi draftot kér (a szerep-ügynök ezt NÉZI ÁT és finomítja, nem nullázza le), majd MINDEN ágon lefuttatja a tényleges `in_progress` állapotváltást -- a draft-kísérlet sosem gátolja a felvételt. Kikapcsolás egy körre: `LOCAL_FIRST_DRAFT=off` env vagy `store/local-first-draft.json {"enabled":false}`; a mező hiánya = ON. -> építi. **(b) Gate-ügynök** (qa/cybersec/cybered): a review után a következő `waiting`+REVIEW kártya, aminek van REVIEW-je de még nincs a saját verdiktje és a hatáskörébe esik (QA=minden kész kártya funkcionálisan; Cybersec=trust-boundary auth/pénz/PII/file/multi-tenant/superadmin/upload; Cybered=magas-tétű publikus-write/auth/superadmin/internet-facing) -> gate-eli. Csak akkor pingelje MikroB-ot, ha nincs neki való munka, vagy valami blokkolt/kétes. **MikroB szerepe marad:** risk-tiering a kétes esetekre, a `done`-ra zárás (CSAK ha minden kijelölt gate PASS/GO -- 4. szabály), fázis-auto-close (5.), a beragadás-figyelés (3.) és Peti. Minden más szabály változatlanul áll (shared-checkout, gate-ek, 8/9. FE-pairing+flow-connectivity, 10. GitHub-first). Így a flotta VAGY dolgozik, VAGY gate-en van, VAGY (üres sor + kvóta) parkol -- soha nem idle-de-MikroB-ra-vár.
...
### mopsion munkakönyvtár a dispatchben (kártya 973ed6eb, a 2513e84d worktree-epic zárása)

A mopsiont (a régi belső néven CleanCore) **minden fejlesztő ügynök a SAJÁT git-worktree-jében** szerkeszti, nem a megosztott klónban. A megosztott klón (`/mnt/h/LM_Studio_Workdir/mopsion`) mostantól CSAK fetch/landolás-alap: oda senki nem commitol. Ez a `shared-file-commit-entanglement` hibaosztály szerkezeti megszüntetése (saját index + saját `core.hooksPath`), nem a fegyelemre bízása.
...
- **Aki csak ELLENŐRIZ** (gate: landolt-e egy sha, mit tartalmaz egy commit), az a fő klónt olvassa `${CLEANCORE_MAIN:-/mnt/h/LM_Studio_Workdir/mopsion}` néven, és **ott nem commitol**. Az `CLEANCORE_MAIN` env-változó neve SZÁNDÉKOSAN nem lett átnevezve a rebranddel együtt (kártya 32dbac1e/876fbf8e) -- csak a fájlnevek és a systemd-egység kapott mopsion-nevet szimlink-kompattal, az env-változó a tényleges kódban ma is `CLEANCORE_MAIN`, ezt a doksi a valósághoz igazodva mondja ki, nem aspirációként. Gate SOHA ne futtasson tesztet más ügynök worktree-jében: ott élő, félkész munka van -- a felülvizsgált SHA-ra nyitott eldobható worktree a helyes hely.
...
### CleanCore -> mopsion rebrand: szimlink-kivezetés feltétele (kártya 32dbac1e/876fbf8e)

A `cleancore-*.sh` -> `mopsion-*.sh` fájlnév-átnevezés (1-2. lépés, kártya 1b02ed3a/647ea02a, landolva 2026-09-19) és a systemd-egység átnevezés (3. lépés, kártya 42749892) szimlink-kompattal ment: a régi név is működik, amíg minden dispatch-szöveg/ügynök-emlékezet át nem áll az újra. A szimlinkek KIVEZETÉSE (a régi nevek végleges törlése) csak akkor biztonságos, ha **minden futó ügynök-session a rename UTÁN indult** -- egy korábban indult session kontextusában/HANDOFF.md-jében még a régi név szerepelhet, és egy kivezetés után az a hivatkozás törne.

**Explicit ellenőrző parancs** (nem "minden bizonnyal újraindultak már" becslés):
```bash
printf 'Authorization: Bearer %s\n' "$(cat store/.dashboard-token)" | curl -s -H @- http://localhost:3420/api/agents | python3 -c "
import json, sys
RENAME_TS = 1789803898  # 2026-09-19 09:44:58 CEST, commit 85602627 (card 647ea02a)
agents = json.load(sys.stdin)
stale = [a['name'] for a in agents if a.get('running') and a.get('runningSince', 0) and a['runningSince'] < RENAME_TS]
print('STALE (pre-rename session, may hold old-name references):', stale) if stale else print('CLEAR: every running session started after the rename')
"
```
Mérve 2026-09-25-én (kártya 876fbf8e): CLEAR -- minden futó ügynök a renaming után indult. A szimlinkek kivezetése ennek ellenére NEM automatikus ebben a kártyában; MikroB dönt a tényleges törlésről (külön lépés), ez a szakasz csak a FELTÉTEL ellenőrzését dokumentálja explicit paranccsal, ahogy a kártya kérte.
...
12. **Szimbólum-jelenlét ellenőrzése SOSEM részsztring, és az azonosító-határ önmagában sem elég (Cybersec javaslata, 2026-09-05, kártyák a14812e8 + e5b7ff19, egy nap, ugyanaz a gyökér; gyógymód-kiterjesztés 06d36307 + 2f0c7d24, ugyanaz a nap).** Amikor egy őr/teszt/eszköz azt állítja, hogy "az X szimbólum jelen van (vagy hiányzik) az Y fájlban", két, egymás után nyíló megkerülési út van. (a) Részsztring-illesztés: az átnevezés megkerüli -- `content.includes('X')` zöld marad, miután `X`-ből `XRENAMED` lett, mert az új név tartalmazza a régit. Az illesztés üljön azonosító-határra (se előtte, se utána `[A-Za-z0-9_$]`). (b) Azonosító-határ: a KOMMENT megkerüli -- töröld a függvényt, hagyj egy kommentet ami megnevezi, és a `present` ellenőrzés örökre zöld marad. Ezért a needle vigye magával a DEKLARÁCIÓS KULCSSZÓT (`function foo`, `def foo`, `class Foo`), ne a csupasz nevet -- ez szűkítés, nem zárás, egy teljes deklarációt idéző komment a kulcsszavas needle-t is zölden tartja. **A komment-megkerülés nem csak csupasz szimbólumnévre áll**: egy kommentbe írt függvényhívás, SQL-utasítás vagy változó-hozzárendelés ugyanígy kielégít egy jelenlét-ellenőrzést, ahol nincs is deklaráció amit needle-be tenni (mérve 2026-09-05: 06d36307 két vektora -- egy `controlMig(...)` hívás és egy SQL `GRANT` kommentben -- és 2f0c7d24 egy változó-hozzárendelés kommentben, három különböző szerzőtől). A gyógymód ezekre általánosan: **az őr a KOMMENTEKTŐL MEGTISZTÍTOTT forráson illesszen**, a szimbólum-eset alá is beleértve. **Ahol NEM alkalmazandó:** próza-/doksi-korpusz, ahol pont bármely említést keresel (a megkülönböztetés: kód-tényt vagy szöveg-tényt állítasz). **Tiltás, méréssel alátámasztva:** ha az őr hamis pozitívot ad, mert a helyes javítás prózája idézi a régi állapotot, NE tegyél bele tagadás- vagy idézőjel-szűrőt -- egy valós korpuszon 61 állításból 8 ült tagadó/idézőjeles soron, és köztük valódi állítások is voltak, egy ilyen szűrő csak hamis negatívot termel; helyette a riport ADDITÍV kategóriát mondjon (pl. "a fejléc hamis" vs "sehol nincs lefedve"), a szűrés helyett a besorolás csökkenti az emberi átnézendő halmazt. **Egy őrt le kell futtatni a saját alapító esetére, mielőtt "nulla lelet"-et jelentesz** -- a nulla ugyanúgy nézhet ki tiszta korpuszként és vak eszközként (a 4. alapelv gyakorlati alakja őrökre).

13. **Üzleti adat CSAK adatbázisban élhet, és a felület MINDIG valós, adatbázisból jövő adatot mutat (Peti szabály 2026-09-26, Telegram 9355/9359).** Minden olyan adat, amit a felhasználó létrehoz, módosít, jóváhagy vagy amire később hivatkozni kell (brigád, hozzárendelés, ajánlat, értékelés, fotó-jóváhagyás, sablon, riport-alap stb.), tartós adatbázisban tárolódik -- memóriában tartott tároló csak teszt/dev fallback lehet, élesben NEM. A kódban a konfiguráció és az üzleti értékek onnan jönnek, ahova ki vannak vezetve (adatbázis, illetve env), nem beégetett konstansból vagy demo-adatból. Egy olyan út, ahol a felhasználó sikeres mentést lát, de az adat újraindításkor elveszik, SÚLYOS hiba (durable-write-lost), nem technikai adósság. Következmények: (a) új tároló/funkció csak Postgres-adapterrel és migrációval (tesztelt rollbackkel, 11. elv) számít késznek; (b) a FE nem mutathat demo/placeholder adatot éles úton -- ha a valós végpont hiányzik, beszédes üres/hiba-állapot kell (12. munkavégzési szabály), nem kitalált adat; (c) a QA gate ellenőrzi: egy éles úton memóriába író tároló vagy demo-adatot mutató képernyő QA FAIL; (d) az éles boot-log `wiring` riportjában minden `durable-write-lost` tétel nyitott, prioritásos kártya, amíg meg nem szűnik.
...
- **Ha nincs README:** hozz létre egy alaposat (lásd a mopsion README mintát: termék, architektúra, repo-térkép, prerequisites, telepítés, env, DB/migráció, futtatás, teszt+gate-ek, security, doksi-index).
...
**Léptékkezelés:** ha egy projekt feature-listája nagy (pl. mopsion, 25+ alszekció, 150+ funkció), a kezdeti feltöltés önálló, dedikált munka -- Fázis/Feladat/alfeladat/lépés bontásban (1. munkavégzési szabály), szekciónként haladva. Ezután minden ÚJ/módosuló funkció a normál definition-of-done része, nem a nagy feltöltés ismétlése.
...
**Kötelező mentés a frissítés ELŐTT (Peti szabály, 2026-09-13, Telegram).** A frissítés csak úgy indulhat, hogy a lokális verzióról ELŐTTE egy checkpoint (mentés) készül -- a sorrend garantált legyen, ne csak utólagos napló. Az `update.sh`-nak a git pull/rebuild/restart ELSŐ lépéseként explicit checkpointot kell rögzítenie (`recovery-prev-version.sh checkpoint` vagy egyenértékű belső hívás), és ha ez a lépés hibázik, a frissítés NE folytatódjon (fail-closed). Végrehajtás: kártya ae071239 (backend, QA+Cybersec gate, mert a rollback-biztonsági utat érinti).

## Óránkénti repo-szinkron: MikroB + mopsion -- SZABÁLY (Peti 2026-09-13, Telegram)

Óránként, de legfeljebb kétóránként ellenőrizni kell mindkét repót (marveen és mopsion): ha fejlesztés történt, ami még nincs landolva, azt landolni kell; ahol a landolt változás éles hatással jár, ott a rebuild és a deploy is mehet. Ez az `oranyi-jelentes` scheduled-task ritmusához igazítva egy önálló, óránkénti (`0 * * * *`) heartbeat-feladat (`repo-sync-deploy`), ami a MEGLÉVŐ eszközökre épül, nem ír újakat:

- **Marveen:** `bash store/marveen-land.sh --all` -- végigsöpri az `agent/*/work` branch-eket, amiket a tulajdonos ügynök még nem landolt saját maga, mindegyiket a saját seam-check + `fleet-test.sh` ellenőrzésén átengedve, csak zöld esetén pusholva `origin/develop`-ra (11. munkavégzési szabály háttér-hálója, már létező mechanizmus). **A MikroB élő szolgáltatást (a sajátunkat) ez a feladat SOHA nem indítja újra és nem futtatja rá az `update.sh`-t** -- az élő rebuild/restart az `update-safety` skill szerint kizárólag Peti explicit jóváhagyásával megy, mert egy önindított restart megölné a saját sessionjét (lásd Rendszerfrissítés-szabály fent). A landolás önmagában elég: a kódváltozás a repóban van, a service-frissítés emberi döntés marad.
- **mopsion:** a `waiting`+gate-PASS/GO, de még nem landolt kártyák (a `gate-reconciler`, 4. munkavégzési szabály már ezt zárja `done`-ra landolással együtt -- ez a sweep csak a réseket fogja be, ha valami csendben kimaradt) landolása a szokásos `mopsion-land.sh <cardId> <gated-sha>` úton (régi név `cleancore-land.sh`, szimlink-kompat). **Deploy csak akkor,** ha a landolt változás ténylegesen éles hatású (nem doksi/teszt-only), ÉS a kártya gate-je (QA + kockázat szerinti Cybersec/Cybered) már lezárt PASS/GO-val ugyanarra a shára (a meglévő `gate-closure-check.py` AGREE-jével igazolva) -- ilyenkor a mopsion.com test-gate deploy mintáját követve: SSH a VPS-re, `git pull` / a landolt kép frissítése, `docker compose up -d` az érintett szolgáltatásra, majd a runbook szerinti smoke-teszt (a kártya saját ellenőrző curl-jai, vagy legalább egy alap health-check). Sikertelen smoke-teszt esetén NE hagyd az élesben félkész állapotot -- vagy a korábbi verzióra állsz vissza (ha van egyszerű visszaállás), vagy azonnal jelzed Petinek Telegramon és nyitva hagyod a kártyát `in_progress`-ben a hibával.
- **Telegram csak akkor,** ha ténylegesen landolt/deployolt valami ebben a körben, vagy ha egy landolás/deploy hibázott -- rutin "nincs mit landolni" kör CSENDBEN marad (heartbeat-stílus, nem `oranyi-jelentes`-szerű kötelező üzenet).
...
**Következmény a telepítőkre MARVEEN-en (a mopsion-szabály NEM változik).** Amíg szimlink, a fenti mopsion-szabály áll: telepítő nem futhat a worktree-ből, mert a közös fát írná át. **Miután `agent-worktree-deps.sh`-val valódi könyvtárrá tetted, ez a tilalom ERRE a worktree-re megszűnik** -- az `npm` ott a sajátját írja, senki máséit. A `--check` megmondja, melyik állapotban vagy; ha nem tudod, futtasd le, ne találgass. A mopsion-oldal (`store/agent-worktree.sh`, per-package linkek) EGYELŐRE VÁLTOZATLAN, ott a tilalom teljes erővel áll.
...
3. `bash store/marveen-land.sh <neved>` -- a saját branch-et egy ELDOBHATÓ, KÜLÖN worktree-ben
   ...
   resetelődik automatikusan landolás után (mint a mopsion landoló scriptje sem teszi) -- szinkronizáld
   kézzel, ha akarod.
