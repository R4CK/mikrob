...
window._i18n.en = {
  ...
  'overview.quota.help.p1':         'The weekly "All models" percentage is checked against a dynamic threshold that depends on how many days remain until the weekly reset: more than 3 days away uses the lowest threshold, under 2 days a higher one, under 1 day the highest — the closer the reset, the more headroom is allowed, since it resets soon anyway.',
  'overview.quota.help.p2':         'Once the weekly percentage reaches the current threshold, the fleet stops starting any NEW development. In-flight cards and their QA/Cybersec/Cybered gates still finish normally, Telegram replies keep working, and monitoring continues.',
  'overview.quota.help.p3':         'Above the threshold the fleet switches to local-LLM offload mode: mechanical, well-scoped coding work is drafted by the local model instead of burning Claude tokens, and every draft is still re-checked online after the reset before it can ship.',
  'overview.quota.help.p4':         'The three sliders below let a superadmin edit those thresholds directly — the change is saved to the fleet config and read by both the dashboard and the dispatch gate script, so they never drift out of sync.',
