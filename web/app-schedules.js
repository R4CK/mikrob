// Schedules page module -- extracted from app.js (card 0159301d / slice 11/N).
// Loaded AFTER app.js in index.html; globals (t, escapeHtml, esc, showToast,
// closeModal, openModal) resolved at call time.
// loadSchedules() is only called from switchPage (pageId=tasks), never at init time.
// Top-level DOM references (scheduleList, saveScheduleBtn etc.) and IIFE event-listener
// blocks execute on module load -- safe because scripts run after full body parse.
...
// === Prompt expand (Save schedule modal subsection) ===
let expandAnswers = []
