...
# SECURITY: 0600 temp header file instead of a curl argv Authorization header --
# /proc/<pid>/cmdline is world-readable, so a bare -H "Authorization: Bearer $(...)"
# leaks the dashboard token to any local user (same class as gate-ops-scripts-token-in-argv,
# b267df80; pattern matches weekly-usage-panel-read.sh).
...
#
# The body is JSON-ENCODED, never assembled by the shell (card 970166ce,
# upstream 9647c965..9abea4ee): BOTH values it reports can carry JSON
# metacharacters. git check-ref-format accepts a double quote in a branch
# name (it rejects space, colon, backslash and '[', but not '"'), and a
# directory name has none of those restrictions, so the repository path can
# hold a quote AND a colon. A quoted branch name would produce a body that no
# longer parses (silent non-enforcement, exactly what this guard exists to
# prevent), and a quote plus a colon in the path could close "content" and
# open a SECOND "to" key, which a parser takes over the first -- delivering
# attacker-written text to an attacker-named agent. Same rule as
# scripts/agent-msg.sh: the values travel in the ENVIRONMENT and json.dumps
# does the quoting.
