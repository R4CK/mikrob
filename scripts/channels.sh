...
if [ -n "$_node_bin" ] && [ -f "$INSTALL_DIR/dist/web/agent-process.js" ]; then
  ...
  if [ -z "$CFG_ENV" ] && [ ! -d "$INSTALL_DIR/.channels-config" ] && [ -s "$INSTALL_DIR/store/.claude-oauth-token" ]; then
    ...
    if [ -f "$INSTALL_DIR/store/.dashboard-token" ]; then
      ...
      rm -f "$_hdr_file"; unset _guard_port _guard_http _hdr_file
    fi
  fi
...
while $TMUX has-session -t "$SESSION" 2>/dev/null; do
  ...
  #
  # Re-fetched HERE, inside the loop (not only once before it at line 1194):
  # this if-block unsets it at the end of every use, so a later iteration
