...
# FORK-OWNED, kept across the upstream adoption (card ca38deac; original card 980454f7).
# Quarantine a stray store/update-health-watchdog.sh. Upstream v1.23.1 replaced it with the
# integrated store/update-finalize.sh; a copy that reappears (untracked leftover, or restored by a
# rollback below the floor commit) re-arms the auto-rollback loop that walked the live install back
# 529 commits on 2026-08-06. Upstream has no equivalent, so taking its file wholesale would drop
# this silently -- it sits before the OS dispatch and overlaps nothing upstream changed.
