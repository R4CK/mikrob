...
OUT="$(mktemp)"; HDR_FILE="$(mktemp)"; chmod 600 "$HDR_FILE"
trap 'rm -f "$OUT" "$HDR_FILE"' EXIT
# SECURITY (card 8418b098): the token never touches argv/ps -- /proc/<pid>/cmdline is world-readable,
# so a bare -H "Authorization: Bearer $(...)" would leak it to any local user on this host.
