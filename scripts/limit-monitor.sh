...
# still followed even when tmux cannot be listed. Fork's own hunk1 comment
# ("Watching only the main channels pane means a sub-agent can hit the wall
# silently") already wanted exactly this -- adopted wholesale, no fork-side
# disagreement.
pane_text() {
...
# Pattern stays the fork's canonical CANDIDATE_RX (session-limit-pattern.sh +
# this monitor's own extra-signal regex, card 115c21e7) -- not upstream's
# inline giant regex, which would drift out of sync with the shared canonical
# source used by src/model-fallback.ts and the other scripts.
CANDIDATE="$(
...
# Both alert paths share ONE contract via send_alert(): honest send, and the
