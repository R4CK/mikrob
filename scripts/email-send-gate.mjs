...
// --- thread-scoped reply (BONIMAIL910, card e3f0e4ed) ------------------------
...
//
// WHAT THIS DOES NOT DECIDE, stated because a reader will assume it does: thread
// membership is derived from headers written by whoever wrote those messages. An
// inbound mail can carry an attacker-chosen Cc or Reply-To, and that address then
// counts as a participant. So the control bounds outbound mail to addresses that
// ALREADY REACHED the mailbox in this thread -- it is not proof that the owner
...
// Throws on ANY failure; per-request timeouts keep the whole check under the
// hook's budget. MEASURED FOR THIS FORK (card e3f0e4ed): injectEmailSendGate
// registers this hook with `timeout: 10`, and the worst case here is two
// SEQUENTIAL 3.5s requests plus two small file reads -- so a real timeout is
// reached by the two AbortSignals, inside the script, where it becomes a DENY.
// That ordering is the whole point: letting the HOOK time out instead would be
// a non-blocking error, i.e. fail-OPEN on the send path.
