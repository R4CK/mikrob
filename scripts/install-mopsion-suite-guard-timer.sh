#!/bin/bash
# Install store/mopsion-main-suite-guard.sh as a systemd --user timer (card 0dadd1e9, renamed from
# install-cleancore-suite-guard-timer.sh in the CleanCore->mopsion rebrand, card 42749892/32dbac1e).
#
...
# silent no-op anyway.
#
# OLD-NAME COMPAT (card 42749892): install-cleancore-suite-guard-timer.sh is now a symlink to this
# file. Whichever name invokes it, it renders/enables the mopsion-named unit -- the point of the
# rebrand is that no NEW cleancore-named unit gets created going forward. The live swap of the
# already-installed mikrob-cleancore-suite-guard unit (stop+disable the old, enable --now this one)
# is a separate, one-time manual step, not something this installer does on every run.
set -euo pipefail
...
GUARD="$STORE/mopsion-main-suite-guard.sh"

[ -f "$GUARD" ] || { echo "install-mopsion-suite-guard-timer: no guard at $GUARD" >&2; exit 1; }
...
NAME="${MAIN_AGENT_ID}-mopsion-suite-guard"
mkdir -p "$UNIT_DIR"
...
[Unit]
Description=mopsion main-suite guard (notice when origin/main goes red)
...
[Unit]
Description=Run the mopsion main-suite guard hourly
