...
those targets came from a single morning's merge of 128 files). NOTE (fork
port, card 907b1a76): this fork's CI (.github/workflows/test.yml) does not run
any scripts/__tests__/*.test.py file today -- that is a pre-existing gap, out
of scope for this port. Run manually: python3 scripts/__tests__/memory-link-audit.test.py
