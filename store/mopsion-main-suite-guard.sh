#!/usr/bin/env bash
# mopsion-main-suite-guard.sh -- notice when the SHARED LOCAL main goes red (card ec44220b).
#
...
#
# Usage:
#   store/mopsion-main-suite-guard.sh            # the scheduled path: silent unless it regressed
#   store/mopsion-main-suite-guard.sh --force    # run even if HEAD is unchanged
#   store/mopsion-main-suite-guard.sh --status   # print the recorded baseline and exit
#
...
die() { echo "RESULT:SETUP-FAILED"; echo "mopsion-main-suite-guard: $1" >&2; exit 3; }
...
if [ ! -e "$TREE/.git" ]; then
  ...
  printf 'Created by store/mopsion-main-suite-guard.sh (card ec44220b). Disposable: this script\nresets and cleans this tree on every run. Do not put work here.\n' >"$MARKER"
elif [ ! -f "$MARKER" ]; then
