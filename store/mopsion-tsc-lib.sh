#!/usr/bin/env bash
# Shared CleanCore measurement helpers, sourced by mopsion-land.sh and mopsion-pregate.sh.
#
...
# (`bash mopsion-tsc-lib.sh --selftest`), never on source -- BASH_SOURCE[0] stays this file's own
...
if [ "${BASH_SOURCE[0]}" = "${0}" ] && [ "${1:-}" = "--selftest" ]; then
  ...
  echo "mopsion-tsc-lib selftest: $n case(s), $([ $fail -eq 0 ] && echo PASS || echo FAIL)"
  exit $fail
fi
