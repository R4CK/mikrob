#!/usr/bin/env bash
# Self-test for store/mopsion-gate-worktree.sh's COLLISION and OWNERSHIP rules (card a7da80d6).
#
# Run: bash store/mopsion-gate-worktree.selftest.sh
...
RUN="$HERE/mopsion-gate-worktree.sh"
pass=0; fail=0
ok()  { printf '  [ok ] %s\n' "$1"; pass=$((pass+1)); }
bad() { printf '  [FAIL] %s\n     %s\n' "$1" "${2:-}"; fail=$((fail+1)); }
