#!/usr/bin/env bash
# mopsion-branch-drift-monitor.sh -- periodic READ-ONLY check for the misdirected-upstream drift
...
#
# REPORTS ONLY WORSENING (same shape as mopsion-main-suite-guard.sh, card 6d46c7d3): a repo that
...
#
# Usage:
#   store/mopsion-branch-drift-monitor.sh            # the scheduled path: silent unless it worsened
#   store/mopsion-branch-drift-monitor.sh --status    # print the recorded baseline and exit
#
# Output contract:
#   STATE:measured
...
# on mopsion-main-suite-guard.sh: an alert path with no isolation override sent real messages to
...
# Atomic write (same reasoning as mopsion-main-suite-guard.sh's state file: a run killed
...
if [ "$first_run" -eq 1 ]; then
  ...
  # "first run has no baseline" case mopsion-main-suite-guard.sh already handles for its own
