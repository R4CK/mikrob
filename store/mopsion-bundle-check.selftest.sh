#!/usr/bin/env bash
# Selftest for mopsion-bundle-check.sh (card 0a907846).
#
...
# guard exists to prevent. It now links dependencies the way mopsion-land.sh does, and ASSERTS the
...
# BEFORE mopsion-tsc-lib.sh is used, exactly as mopsion-land.sh sets it.
MAIN="$CC"
. "$HERE/mopsion-bundle-check.sh"
. "$HERE/mopsion-tsc-lib.sh"
...
if [ "${CC_BUNDLE_SELFTEST_REAL:-0}" = 1 ]; then
  ...
  for probe in "46356720:expect-fail" "e80945dc:expect-pass"; do
    ...
    # production path uses (mopsion-land.sh calls exactly this before its own bundle check).
