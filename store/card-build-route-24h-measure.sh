...
[ -f "$LOG" ] || { [ "$JSON" = 1 ] && printf '{"hours":%s,"dispatches":0,"capacity_skipped":0,"content_considered":0,"drafted":0,"continued_with_draft":0,"rejected_draft":0,"exhausted_no_draft":0,"pending_review":0,"dispatcher_self_advance":0,"dispatcher_mikrob_dispatch":0,"dispatcher_unattributed":0}\n' "$HOURS" || printf 'card-build-route-24h-measure: no log at %s -- nothing to measure yet\n' "$LOG"; exit 0; }
...
# C section 4b step (dispatcher=mikrob-dispatch) or a role-agent's self-advance-pickup.sh
...
while IFS=$'\t' read -r card verdict path ts dispatcher_field; do
  ...
  if printf '%s' "$path" | grep -Eq "$CAPACITY_REGEX"; then
    ...
    case "$dispatcher_field" in
      ...
      dispatcher=mikrob-dispatch) dispatcher_mikrob_dispatch=$((dispatcher_mikrob_dispatch + 1)) ;;
...
if [ "$JSON" = 1 ]; then
  printf '{"hours":%s,"dispatches":%s,"capacity_skipped":%s,"content_considered":%s,"drafted":%s,"continued_with_draft":%s,"rejected_draft":%s,"exhausted_no_draft":%s,"pending_review":%s,"dispatcher_self_advance":%s,"dispatcher_mikrob_dispatch":%s,"dispatcher_unattributed":%s}\n' \
    ...
    "$dispatcher_self_advance" "$dispatcher_mikrob_dispatch" "$dispatcher_unattributed"
else
  ...
  printf '  dispatcher (kartya 3906d77b): %s mikrob-dispatch, %s self-advance, %s unattributed (regi sor vagy nem-allitott env)\n' \
    "$dispatcher_mikrob_dispatch" "$dispatcher_self_advance" "$dispatcher_unattributed"
  if [ "$content_considered" -gt 0 ] && [ "$dispatcher_self_advance" -eq 0 ] && [ "$dispatcher_mikrob_dispatch" -gt 0 ]; then
    printf '  !! minden tartalmi dontes mikrob-dispatch-bol jott, EGY sem self-advance-bol -- ez pontosan az a res, amiert a 3906d77b nyilt (09-18 08:41 utan a self-advance uton felvett kartyak nem hivtak a routert). Ellenorizd, hogy a role-agentek self-advance-pickup.sh-t hivjak-e PUT in_progress helyett.\n'
  fi
