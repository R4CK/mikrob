...
# Native-binding health check (card 5b06720f, corrected per Cybered NO-GO komment 6091). A
# foreign package manager (pnpm/yarn) run in the shared checkout can silently move node_modules
# aside and drop better-sqlite3's compiled binding -- measured incident: this failed ALL 786
# files with a "Could not locate the bindings file" error repeated once per file, instead of one
# speaking failure up front.
#
# A bare `require('better-sqlite3')` is BLIND to exactly this failure: better-sqlite3 11.10.0
# loads its native binding LAZILY, inside the Database constructor (lib/database.js), not at
# require time -- so the require succeeds even with the binding missing/broken. The check has to
# actually open a database to exercise that load.
if ! NATIVE_CHECK_ERR="$(node -e "new (require('better-sqlite3'))(':memory:')" 2>&1)"; then
