#!/usr/bin/env bash
# mopsion-bundle-check.sh -- does the merge result still produce a BROWSER BUNDLE? (card 0a907846)
#
...
bundle_filters_or_die() {
  ...
  if [ "$#" -eq 0 ]; then
    echo "mopsion-bundle-check: CC_BUNDLE_FILTERS resolved to zero filters -- refusing rather than silently skipping every build (raw value: '$CC_BUNDLE_FILTERS')" >&2
    exit 1
  fi
}
