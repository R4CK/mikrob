#!/usr/bin/env bash
# mopsion-pregate.sh -- the two cheap measurements nobody ran, before a card reaches a gate
...
#
# Usage:  mopsion-pregate.sh <cardId> <gated-sha> [--no-tests]
#         mopsion-pregate.sh --selftest
...
CARD="${1:-}"; SHA="${2:-}"; MODE="${3:-}"
[ -n "$CARD" ] && [ -n "$SHA" ] || { echo "usage: mopsion-pregate.sh <cardId> <gated-sha> [--no-tests]" >&2; exit 2; }
...
# its filter, which is worse than a slow one. Same derived rule mopsion-land.sh already uses: the
