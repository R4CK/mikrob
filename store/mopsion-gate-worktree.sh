...
#
#   store/mopsion-gate-worktree.sh --agent <you> <card> <sha>     # create or top up; prints the path
#   store/mopsion-gate-worktree.sh --agent <you> --path <card> <sha>
#   store/mopsion-gate-worktree.sh --agent <you> --remove <path>   # stop YOUR strays there, then remove
#
...
die() { echo "mopsion-gate-worktree.sh: $2" >&2; exit "$1"; }

# `--agent` and `--force` may appear anywhere, so a caller can put the agent first (matching
# agent-worktree.sh / mopsion-suite-run.sh) or last, without the meaning changing.
ARGS=()
while [ $# -gt 0 ]; do
  case "$1" in
...
if [ "${1:-}" = "--remove" ]; then
  target="${2:-}"
  [ -n "$target" ] || die 2 "usage: mopsion-gate-worktree.sh --remove <path>"
...
CARD="${1:-}"; SHA="${2:-}"
[ -n "$CARD" ] && [ -n "$SHA" ] || die 2 "usage: mopsion-gate-worktree.sh <card> <sha> | --path <card> <sha> | --remove <path>"
