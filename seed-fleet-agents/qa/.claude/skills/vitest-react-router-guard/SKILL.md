...
description: Vitest tests for React Router v6 components, including auth/role guards that render <Navigate>. Covers: guard infinite-redirect-loop, vi.hoisted mock pattern for useSession and for data constants + vi.fn, nested vi.mock hoisting override, useParams mutable-variable mock, multiple-elements-same-text scoping, JSDOM aria-landmark pitfalls (<header> inside <main>), i18n translation value vs key mismatch. Trigger: "test a route guard", "test PortalAuthGuard / AuthGuard / PrivateRoute", "guard test hangs / infinite loop", "vi.mock useSession", "vi.mock useParams", "useParams returns empty", "TDZ vi.hoisted", "multiple elements with same text", "Found multiple elements", "multiple elements with role banner", "getByRole banner fails".
---
