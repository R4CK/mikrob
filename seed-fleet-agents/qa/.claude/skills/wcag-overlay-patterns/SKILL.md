...
- **Inline modal missing focus trap (QA WCAG SC 2.1.2 FAIL risk)**: When a dialog is implemented as conditional JSX inside a host component (not as a dedicated BottomSheet/DrawerPanel), the focus trap useEffect is NOT automatically inherited -- you must manually add it every time. Common pattern: TOTP confirmation modals, inline delete-confirm dialogs. Check with `grep -n "role=\"dialog\"\|useEffect\|keydown" <file>` -- every `role="dialog"` must have a corresponding `keydown` listener and `ref` pointing to the inner container. If missing: QA WCAG SC 2.1.2 FAIL.
- **Nullable open-state dependency**: When the "open" trigger is nullable (e.g. `SeatBand | null`) rather than boolean, key the effect on the nullable value: `useEffect(() => { if (totpBand === null) return; ... }, [totpBand])`. The early return handles the "closed" case; the effect fires when the value transitions null → non-null (open) and non-null → null (close/cleanup).
