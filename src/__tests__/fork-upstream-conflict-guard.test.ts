...
const ACKNOWLEDGED_CONFLICTS = {
  ...
  'scripts/hooks/outgoing-copy-gate.py':
    ...
    " Round 13 (2026-09-03, card 3bd18e70 landing-unblock, blob c724df596611..d35afdd048eb): upstream changed the POLICY for a missing rules file (CLCOPYGATEHIANY902, owner decision on upstream side): load_bad_name() now returns (regex, state) with state ok/missing/empty/invalid, missing/empty -> email fail-OPEN with a loud systemMessage on every send, invalid (file exists but unparseable/bad schema/bad regex) -> fail-CLOSED as before. NOT adopted this round: the fork file stays wholesale (its email path remains fail-closed on a missing file, which is the fork owner's standing choice, not a bug), so this is acknowledge-only again. The missing-vs-invalid split is a real adoption candidate that needs an explicit owner decision on the fork side (it relaxes a fail-closed gate), not a rider on a dashboard landing.",
