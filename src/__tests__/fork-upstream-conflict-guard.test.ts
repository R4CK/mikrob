...
const ACKNOWLEDGED_CONFLICTS = {
  ...
  'src/web/context-restart-gate-runner.ts':
    ...
    " Re-measured 2026-09-06 (backend3, card 79bb0364 round-3 landing-block, upstream round 17). The other half of the same change: an unanswered inbound now holds the gate shut only until the agent has actually been SHOWN it, read from the ledger drain's own state file. Upstream's reason is measured -- a one-word 'ok' held a gate for eight hours at 630% of the threshold. Real and adoptable, and it needs src/db.ts's new export, so the two are adopt-together or neither. The agent-config/claude-plans import pair this rule decides is untouched. Resolution unchanged; blob bumped.",
  ...
  'src/web/agent-scaffold.ts':
    ...
    " Re-measured 2026-09-06 (backend3, card 79bb0364 round-3 landing-block, upstream round 17). Upstream added a CROSS-SCOPE duplicate-hook check: Claude Code merges the user scope with the project scope and runs BOTH without deduping, so the same gate fired twice per prompt (measured -- a doubled process spawn and a doubled ~1.4KB context injection, and removing the entry by hand did not hold because the next dashboard start merged it back). It compares SCRIPT BASENAME rather than the command string, because the two scopes legitimately spell the same gate differently, and it is deliberately ONE-WAY. This fork wires several python guards through this file, so the duplicate class plausibly applies here too -- worth its own card rather than a landing-unblock rider, and the 'not safe to hand-merge under time pressure' warning on this entry still stands. Resolution unchanged; blob bumped.",
  ...
  'src/db.ts':
    ...
    " Re-measured 2026-09-06 (backend3, card 79bb0364 round-3 landing-block, upstream round 17). Upstream added openInboundQuestionMessageId() next to hasOpenInboundQuestion() -- same rule, but it hands back WHICH message, so a caller can ask whether the agent has already been shown it. Purely additive, 27 lines, and it is the DB half of the context-restart-gate change in the sibling entry below; the two go together. Nowhere near moveKanbanCard(), which is what this rule decides. Not adopted this round; resolution unchanged, blob bumped.",
  ...
  'src/web/routes/updates.ts':
    ...
    " Re-measured 2026-09-06 (backend3, card 79bb0364 round-3 landing-block, upstream round 17). Upstream factored the rev-count helper out and added behindCount() plus originHasBranch(), the latter distinguishing `git ls-remote` exit 2 ('no such branch', evidence) from 128 ('transport or auth failure', an unknown you must not block on) -- and treating an undefined status as unknown too. That distinction is exactly the fail-closed-vs-unknown care this fork applies elsewhere. Additive; not adopted this round. Blob bumped.",
  ...
  'scripts/hooks/outgoing-copy-gate.py':
    ...
    " Re-measured 2026-09-06 (backend3, card 79bb0364 round-3 landing-block, upstream round 17). Upstream gave the gate log TIMESTAMPS. The reason is the sharp part and it is the same class this fork keeps finding: 8005 log lines, four distinct messages, and one of them recorded a FAIL-OPEN pass-through on the Telegram branch -- a message that went out unaudited -- with no way to tell which day it happened on. A log whose entries cannot be placed in time cannot be used to check anything. Cheap and adoptable, and it does not touch load_bad_name(), which is what this entry's resolution is about. Not adopted inside an unblock; noted as the strongest small candidate here. Blob bumped.",
  ...
  'scripts/channels.sh':
    ...
    " Re-measured 2026-09-06 (backend3, card 79bb0364 round-3 landing-block, upstream round 17). Upstream made resolve_main_model()'s failures NAMED instead of silently empty: a missing node, a missing or stale dist, or an empty read now each write a line to store/channels-failures.log, and node is looked up in the standard install locations because the launchd PATH is narrow at that point. Same absent-is-not-zero direction this fork argues for elsewhere. It does not touch the 0600-temp-file header pattern or the honest-delivery check this rule decides. Not adopted; blob bumped.",
  ...
  'src/web/channel-monitor.ts':
    ...
    " Re-measured 2026-09-06 (backend3, card 79bb0364 round-3 landing-block, upstream round 17). Upstream added the post-restart RECOVERY BRIEF (card 3a64403b): after a watchdog restart the agent comes back on a fresh session while its uncommitted branch and in_progress card wait for it, so the monitor types a short brief into the new session. The decision of whether to speak lives in a pure buildRecoveryBrief (no facts -> no message), which is the right shape. It is ANOTHER new file (restart-recovery-brief.ts) plus a listKanbanCards import, i.e. a feature, not a fix. Not adopted; the triggerMarveenMemorySave hunk and the STUCKINPUT827 residual this rule names are untouched. Resolution unchanged; blob bumped.",
...
const ACKNOWLEDGED_UPSTREAM_BLOBS: Readonly<Record<keyof typeof ACKNOWLEDGED_CONFLICTS, string>> = {
  ...
  'src/web/agent-scaffold.ts': '5b168b5afc24e50ffca5be0d06de1161027e7a9a',
  'src/db.ts': 'c554b375fcabf879fb324789459c72608baf9b5c',
  ...
  'scripts/hooks/outgoing-copy-gate.py': 'd97e9683b3ca0582879ca323436f5ab994ad6ff9',
