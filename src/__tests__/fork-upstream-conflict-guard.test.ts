...
const ACKNOWLEDGED_CONFLICTS = {
  ...
  'src/web/agent-process.ts':
    'union all three: (1) ISOLATED_CONFIG_SKIP keeps BOTH \'skills\' (fork) and \'projects\' (upstream) entries; (2) ADOPTED 2026-09-04 (card e80c011a): upstream\'s resolveProviderEnv() refactor, MINUS the minimax branch. Verified byte-identical output for claude/ollama/deepseek/openrouter incl. the b7fa5281 shSingleQuote fix -- the test reimplements the pre-refactor inline expressions and asserts byte equality, so the no-op claim is measured, not asserted. MiniMax is EXCLUDED ON PURPOSE: Peti gave it a NO-GO on card 48565f81 (CLAUDE.md rule 17), and the earlier wording here -- "adopt wholesale ... plus adds minimax" -- would have landed a declined feature as a side effect of a refactor. src/__tests__/provider-env-adoption.test.ts pins the exclusion behaviourally AND at source level so a later upstream sync cannot bring it back quietly; (3) DO NOT adopt upstream\'s umask 002 + agentTmuxTarget(name)/startTarget change. MikroB decision 2026-09-04 (cards bd450735/e80c011a). The earlier text here called it a "verified no-op ... adopt wholesale", and that sentence was wrong in the way that matters: the OUTPUT is a no-op, the DEPENDENCIES are a feature. Measured -- it needs readAgentRunAsUser(), a 5th runAsUser parameter plus a sudo branch in buildTmuxInvocation (ssh-tmux.ts), sessionRunAsUserMap() with a TTL cache, runAsUserForTmuxArgs(), resolveTarget(), a HOST-LEVEL SUDOERS RULE (upstream says so in its own comment), and generalising 23 runTmux( call sites in the fleet\'s most startup-critical file. The fork has NO runAsUser consumer: the word does not appear anywhere in src/. So this is per-agent OS-user isolation with nothing using it, and umask 002 is dead without it (it fires only when runAsUser is set). If the fleet ever wants that isolation it is its OWN card with its own plan-grilling, because a sudoers change is host-level and hard to reverse -- not a line item inside a reconciliation -- the fork\'s *Unlocked/withLifecycleLock split (card 74ba7c78) is UNRELATED to this hunk set and already correctly merged, do not touch it (4) Re-read 2026-09-03 (card 3bd18e70, blob 4c439228): the agent-scaffold import line now conflicts too -- union the fork ensureLocalFirstSection with upstream ensureSystemDirectiveAuthSection (GUARDHITELES903) on one line; the ensureSystemDirectiveAuthSection(name) call in startAgentProcess auto-merges (additive).' +
