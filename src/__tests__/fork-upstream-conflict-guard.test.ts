...
const ACKNOWLEDGED_CONFLICTS = {
  ...
  'src/db.ts':
    ...
    " Re-measured 2026-09-04 (backend2, card e4d12956, 6a71eab9ab67..cf4c1052f7ef): ONE hunk, entirely inside saveMemory() -- upstream now generates the embedding on this path too, capturing lastInsertRowid and firing generateEmbedding().then(UPDATE memories SET embedding). Nowhere near moveKanbanCard(); resolution unchanged, blob bumped. WORTH FLAGGING FOR THE B-WAVE, NOT FOR THIS RESOLUTION: measured on the fork, our own saveMemory() (src/db.ts ~1489) has the SAME gap upstream just closed -- it INSERTs without embedding, while saveAgentMemory and the agent-memory path do embed. Upstream's own comment names the consequence and it applies to us verbatim: the nightly daily-log digest is written through saveMemory, so those rows stay out of semantic search. That is an ADOPTION decision on a B-wave file, not a passive conflict resolution, so it is deliberately NOT folded in here.",
