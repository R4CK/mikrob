...
const ACKNOWLEDGED_CONFLICTS = {
  ...
  'scripts/hooks/outgoing-copy-gate.py':
    ...
    " Round 13 (2026-09-03, card f4442719 upstream/develop merge, c724df596611..d35afdd048eb): upstream (CLCOPYGATEHIANY902, owner decision TG 14442) rewrote load_bad_name() into a four-state model (ok/missing/empty/invalid): a MISSING or EMPTY rules file now fails OPEN with a loud, user-visible systemMessage warning on every send (was: silent fail-closed for missing, silent fail-open for empty on the fork's own sentinel), while an INVALID file (present but malformed JSON/schema/regex) stays fail-CLOSED, on the reasoning that a customer with no rules file configured should not lose outbound email entirely over a check they never set up. NOT adopted this round: this is a genuine security-posture change (missing-file behavior flips from fail-closed to fail-open) that lands squarely on the same open question as kanban card 36a456e3 (fill store/outgoing-copy-gate-rules.json's bad_name_patterns with real patterns, or knowingly leave it empty) -- adopting it here, inside an unrelated infra merge, would decide that question by side effect instead of by Peti's actual answer. Acknowledge-only; a real decision (and, if adopted, an 'invalid'-state distinction the fork's current None/sentinel split does not yet have) is a candidate for a dedicated follow-up once 36a456e3 resolves, not a rider on this merge.",
