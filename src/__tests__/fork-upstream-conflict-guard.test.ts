...
const ACKNOWLEDGED_CONFLICTS = {
  ...
  'src/web/token-usage.ts':
    ...
    " Re-measured 2026-09-05 (mikrob, landing-block): upstream 346fa63739d8..82ebcf785cd0 adds resolvesToSharedProjectsRoot() + a skip check in discoverAgentSources, closing a measured triple-count on upstream's own install (the SAME transcript reached their cost parser under three symlinked paths, because their cursor table is keyed by file path). CHECKED whether that applies here before assuming the earlier no-op note still holds: it does, but not for the reason the note gives -- this file's own dedup key is `(agent, session_id, timestamp, input, output)` (content), not a file path, so the same transcript under three symlinked paths still collapses to one row via INSERT OR IGNORE regardless of which path found it. Upstream's fix and this fork's existing guard solve the identical failure by different mechanisms; neither needs the other. Not adopted (no bug to close here); resolution unchanged. Blob bumped.",
...
const ACKNOWLEDGED_UPSTREAM_BLOBS: Readonly<Record<keyof typeof ACKNOWLEDGED_CONFLICTS, string>> = {
  ...
  'src/web/channel-monitor.ts': 'ab96c868f316cfb4ca430fc9c24a822677187c68',
