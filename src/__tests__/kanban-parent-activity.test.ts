...
describe('parent updated_at follows subcard activity', () => {
  ...
  it('terminates on a parent_id cycle, and says it was a cycle', () => {
    // DEVIATION FROM ACKNOWLEDGED_CONFLICTS (card 4f15966e, backend, 2026-09-07): this upstream test
    // arrived non-conflicting, written against upstream's own console.warn(`[kanban] ...`) call
    // shape. The fork's conflict resolution deliberately kept logger.warn({data}, 'message') instead
    // (structured logging, CLAUDE.md engineering-standards) -- spying on console.warn here would
    // never observe it. Spy on logger.warn and read the message from the SECOND argument instead.
    ...
    try {
      ...
      expect(messages.some((m) => m.includes('too deep'))).toBe(false)
    } finally {
      warn.mockRestore()
    }
  })
  ...
  it('stops at the depth limit on a chain longer than any real hierarchy, and says so', () => {
    ...
    try {
      ...
      const messages = warn.mock.calls.map((c) => String(c[1]))
      expect(messages.some((m) => m.includes('too deep'))).toBe(true)
