...
// Fork-owned slices, not app.js: every overlay below (plus the shared
// overlayHasUnsavedInput/attachOverlayCloseGuard helpers) lives in an extracted
// slice file (modularisation, see the STUB comments in app.js), which post-dates
// this test's origin upstream. app-memories.js hosts the shared helpers because
// it is the first script index.html loads -- every later overlay file calls
// attachOverlayCloseGuard() at its own top level, so the definition must already
// exist (card 72f5f13b merge port).
const SRC_FILES = [
  'app-memories.js', 'app-kanban.js', 'app-elements.js', 'app-schedules.js',
  'app-connectors.js', 'app-import-migration.js', 'app-skills.js', 'app-archive-timeline.js',
]
const src = SRC_FILES.map((f) => readFileSync(join(webDir, f), 'utf8')).join('\n')
