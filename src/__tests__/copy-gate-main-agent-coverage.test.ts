...
// DEVIATION FROM ACKNOWLEDGED_CONFLICTS (card 4f15966e, backend, 2026-09-07): upstream's
// TELEGRAM_COPY_GATE_MATCHER export (agent-scaffold.js) is not adopted -- the fork wires the same
// outgoing-copy-gate.py script under this exact matcher string via its own mechanism (card 74181db2),
// which this test's whole point is to verify against the committed .claude/settings.json. Inlined
// as a literal rather than imported, since the exporting constant does not exist in this fork.
const TELEGRAM_COPY_GATE_MATCHER = 'mcp__plugin_telegram_telegram__reply|mcp__plugin_telegram_telegram__edit_message'
