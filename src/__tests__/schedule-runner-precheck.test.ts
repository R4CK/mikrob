...
describe('schedule-runner pre-check integration (source-level)', () => {
  ...
  it('uses fail-open semantics (no SKIP on non-zero exit, no throw on missing file)', () => {
    const start = SRC.indexOf('export async function runPreCheck')
    expect(start, 'runPreCheck not found -- an empty slice would make every assertion below vacuous').toBeGreaterThan(-1)
    const preCheckFn = SRC.slice(start)
