...
// FORK DELTA (card ab4c85f2), the only difference from upstream's copy of this
// file: upstream declares the mock's four parameters with a leading underscore
// and relies on an argsIgnorePattern this fork's eslint config does not set
// (@typescript-eslint/no-unused-vars is a bare 'error' here), so upstream's
// shape costs four lint findings and the ratchet refuses the landing. The call
// signature moves into the generic instead -- the argument types the assertions
// read (`mock.calls[0][1]`) are preserved, and nothing is declared unused.
type SendPrompt = (session: string, text: string, host?: string | null, opts?: unknown) => Promise<Outcome>
const sendPromptToSession = vi.fn<SendPrompt>(
  async () => {
    calls.push('inject')
    return 'sent'
  },
