...
// SKIPPED (2026-09-03, card f4442719 upstream merge): this whole suite pins upstream's
// OLLAMA_URL-precedence/probe/embedding-pull rewrite of install-linux.sh's silent Ollama
// auto-install step. That step does not exist on this fork at all -- Peti's 2026-08-13
// directive (EPIC ebc7b4dd, see store/first-run-llm.sh's header) intentionally removed it:
// the runtime is offered-never-silent, the embedding model is a separate automatic
// dependency, and the coding model comes from an explicit user-picked catalogue. This is not
// a gap this merge introduced -- it is the SAME decision already recorded, twice, in
// fork-upstream-conflict-guard.test.ts's ACKNOWLEDGED_CONFLICTS for 'install-linux.sh' and
// the companion (deleted) test file (card 9dc0fba8, MikroB approval 2026-09-02): "the Ollama
// half is unchanged... there is still nothing to graft THERE." Un-skipping this suite would
// mean resurrecting a feature the fork has explicitly, repeatedly declined across multiple
// upstream-merge rounds -- a policy reversal, not a merge-conflict resolution.
describe.skip('installer: OLLAMA_URL resolution matches the runtime', () => {
