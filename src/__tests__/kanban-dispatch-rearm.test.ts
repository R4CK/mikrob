...
async function move(id: string, status: string, actor?: string, force?: boolean): Promise<void> {
  const req = Readable.from([Buffer.from(JSON.stringify({ status, sort_order: 0, actor, force }))]) as unknown as http.IncomingMessage
...
describe('kanban dispatch re-arm', () => {
  ...
  it('re-arms from waiting too (an escalated card that gets picked up again)', async () => {
    ...
    // Fork-only gate (card c4f2de32): reopening a `waiting` card into in_progress without a
    // gate-verdict comment is blocked unless `force` -- this card has no comments at all, so the
    // reopen needs the explicit override to reach fireKanbanDispatch at all.
    await move('card-4', 'in_progress', 'orin', true)
    expect(mockCreateAgentMessage).toHaveBeenCalledTimes(2)
  })
})
