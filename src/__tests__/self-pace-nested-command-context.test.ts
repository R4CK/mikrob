...
describe('self-pace-gate: bash GRAMMAR -- what establishes COMMAND POSITION (card 84e31b40, F-9)', () => {
  const NL = String.fromCharCode(10)
  const bash = (command: string) => gateDecision('Bash', { command })
  const CT9 = 'cron' + 'tab'
  const EVIL9 = ['import subprocess', `subprocess.run(['${CT9}', '-'])`].join(NL)
  // Prose that WOULD trigger the gate if the heredoc body were not blanked.
  const HOT9 = `{"content":"ne ${CT9} -e -t hivast irjunk, hanem a scheduler API-t"}`
