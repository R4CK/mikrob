...
//  2. BEHAVIOUR -- the real bridgeEnrollErrorText, evaluated with a stub t():
//     known code translates, unknown code falls back to the server's sentence,
//     and nothing renders a bare key. Read from web/app-settings-auth.js, NOT
//     web/app.js: this fork extracted the auth panel into that slice (slice 27),
//     and the fork's own app.js rule is that an upstream app.js hunk is diffed
//     against the named slice. Reading app.js here would find nothing and the
//     `start > -1` assertion below is what would say so.
...
describe('BRIDGEHU813 -- the dashboard translates on the code, not on the sentence', () => {
  ...
  function loadErrorText(dict: Record<string, string>) {
    const app = readFileSync(join(ROOT, 'web/app-settings-auth.js'), 'utf-8')
    const start = app.indexOf('function bridgeEnrollErrorText(data) {')
    expect(start, 'bridgeEnrollErrorText not found in web/app-settings-auth.js').toBeGreaterThan(-1)
  ...
  // FORK ADDITION. Upstream pins the WIRING with its browser suite (tests/browser/**),
  // which this fork did not adopt -- so without this, reverting the error branch to
  // `data.error` would leave every assertion above green while the user saw English
  // again, which is the exact bug this card fixes. A translator nobody calls is not a
  // translation. Measured: with the branch reverted, this is the only test that fails.
  it('the error branch actually CALLS it (the function alone guarantees nothing)', () => {
    const src = readFileSync(join(ROOT, 'web/app-settings-auth.js'), 'utf-8')
    const start = src.indexOf('async function bridgeEnrollFromUi()')
    expect(start, 'bridgeEnrollFromUi not found').toBeGreaterThan(-1)
    const body = src.slice(start, src.indexOf('\n}', start) + 2)
    expect(body, 'the !r.ok branch must translate on the code').toContain('bridgeEnrollErrorText(data)')
    expect(body, 'the raw server sentence must no longer be assigned directly').not.toMatch(
