...
const EXEMPT: Record<string, string> = {
  ...
  'noisy-command-guard.py':
    'wired (PreToolUse, Bash matcher) in the operator\'s own ~/.claude/settings.json, outside every scanned REGISTRATION_SURFACE -- confirmed live 2026-09-03',
  'big-file-guard.py':
    'wired in the operator\'s own ~/.claude/settings.json, outside every scanned REGISTRATION_SURFACE -- confirmed live 2026-09-03',
  'secret-write-guard.py':
    'wired in the operator\'s own ~/.claude/settings.json, outside every scanned REGISTRATION_SURFACE -- confirmed live 2026-09-03',
  'shared-memory-inject.py':
    'wired (SessionStart) per-agent in agents/<name>/.claude/settings.json -- gitignored (agents/), so no REGISTRATION_SURFACE can see it; confirmed live in agents/backend/.claude/settings.json 2026-09-03',
  'activity_memory_capture.py':
    'the file\'s own header: "Prototype for ONE agent (backend) -- card 4829ccff. NOT in the fleet-wide template." -- confirmed NOT wired even in agents/backend/.claude/settings.json; a genuine pre-existing gap (wire it or drop the prototype), not something this merge introduced or should silently decide',
}
...
function hookScripts(): string[] {
  return readdirSync(HOOKS_DIR)
    ...
    // A *.selftest.py is the test suite for the hook next to it, never itself a production
    // hook -- same non-hook character as ledger_lib.py/clearstate_lib.py below, just named by
    // a suffix instead of listed one-by-one.
    .filter((f) => !f.endsWith('.selftest.py'))
    .sort()
}
