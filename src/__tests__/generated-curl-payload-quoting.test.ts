...
describe('generated autonomy block: no shell-expanding curl payload', () => {
  it('the measure is not vacuous -- the written block really carries curl examples', () => {
    ...
    // FORK ADAPTATION (B-wave, card 42938a74): this fork's generated examples read the token from
    // STDIN (`printf 'Authorization: Bearer %s\\n' "$(cat <token>)" | curl -s -H @- ...`) so the
    // token never reaches the command line, where /proc/<pid>/cmdline would expose it to any local
    // process. The line therefore STARTS with `printf`, not with `curl`.
    const curlLines = block.split('\n').filter((l) => /(^|\|\s*)curl\s/.test(l.trimStart()))
  ...
  // FORK ADAPTATION (B-wave, card 42938a74). Upstream pins `--data-binary @- <<'JSON'` here. That
  // exact form is NOT reachable in this fork, and the reason is a second protection upstream does
  // not have: `-H @-` already occupies stdin so the token stays out of argv, and `--data-binary @-`
  // wants the same stdin. The two cannot both use it. So the fork keeps BOTH protections by a
  // different route -- token on stdin, payload SINGLE-QUOTED (the shell expands nothing inside
  // single quotes, which is the property the heredoc was bought for) -- and the warning directs a
  // payload that needs a variable to a file written by python3+json.dumps. Asserted as the two
  // properties rather than as upstream's spelling, so a regression in either still fails.
  it('the level 1 example keeps the payload inert AND the token out of argv', () => {
    const block = writtenAutonomyBlock('heredoc-agent')
    const level1 = block.slice(block.indexOf('Level 1'), block.indexOf('Level 2'))
    const send = level1.split('\n').find((l) => l.includes('/api/messages') && l.includes('curl'))
    expect(send, 'level 1 send example not found').toBeTruthy()
    expect(send!).toContain(`-d '`)
    expect(send!).not.toMatch(SHELL_EXPANDING_PAYLOAD)
    expect(send!).toContain('-H @-')
    expect(send!).not.toMatch(/-H\s+"Authorization/)
  })
...
describe('generateClaudeMd source: no shell-expanding curl payload', () => {
  ...
  it('the measure is not vacuous -- the generator body is found and carries curl examples', () => {
    ...
    expect(src.slice(fnStart, fnEnd)).toContain('curl -s -H @- -X POST')  // fork: token on stdin
  })
  ...
  // FORK ADAPTATION (B-wave, card 42938a74), same reason as the level-1 case above: `-H @-` holds
  // stdin for the token, so `--data-binary @-` cannot also have it. Both protections are asserted
  // as properties instead -- inert payload AND token off the command line -- plus the warning that
  // an UNQUOTED heredoc is the same hazard, which is the half a reader is most likely to get wrong.
  it('the stranger-sender block keeps the payload inert AND the token out of argv', () => {
    const body = src.slice(fnStart, fnEnd)
    ...
    expect(block).not.toMatch(SHELL_EXPANDING_PAYLOAD)
