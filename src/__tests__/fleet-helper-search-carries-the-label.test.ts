...
function run(label: string, rows: unknown[], kwargs: Record<string, unknown>) {
  ...
  const stdout = execFileSync(
    ...
    // PYTHONDONTWRITEBYTECODE: fleet.py lives under seed-skills/, a SHIPPED template tree. Without
    // this, importing it writes __pycache__/*.pyc into that tree, and template-identity-hygiene.
    // test.ts -- which walks the same tree and reads every file as utf-8 -- then reports the
    // decoded bytes as a hardcoded absolute home path. The suite fails on its own artifact, and
    // only when this file happens to run first, so it reads as a flake (adopted from upstream,
    // card 123983f3).
    {
