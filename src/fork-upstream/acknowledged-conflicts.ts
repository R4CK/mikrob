...
export const ACKNOWLEDGED_CONFLICTS = {
  ...
  'scripts/watchdog.sh':
    'ADOPT upstream (agent=${AGENT_ID} query param + HTTP_CODE capture with early return on non-200). The fork\'s OWN adjacent Hungarian comment (unrelated to this conflict, sitting right above it) already documents the exact bug this fixes: "agent, nem to: a to SOSEM volt ismert parameter ezen a vegponton... es 2026-08-18 ota HANGOS 400-at kap" -- yet the fork\'s HEAD-side conflict content still used the stale to=${AGENT_ID} param, contradicted by its own neighbouring comment. Combined with the same comment\'s warning that curl without -f returns 0 on a 4xx and the error-JSON is neither empty nor "[]" (so it flows to the replay logic AS DATA), upstream\'s explicit HTTP_CODE!=200 guard is the correct, and evidently already-intended, fix.',
  'scripts/install-prod-tree-guard-hook.sh':
    ...
    " Re-measured 2026-09-25 (backend3, card b5b7eb6b child 09d54e88, 9abea4ee17d6..68232de630af): plain blob bump -- both hunks land exactly on the two points this rule already decides. (1) the JSON-encoding comment: HEAD and upstream say the SAME thing (upstream fixed this in an earlier round the fork already carries, cited as 'upstream 9647c965..9abea4ee' in the fork's own comment); upstream's current text is the same explanation minus the retrospective citation. Kept the fork's version (superset: identical content + provenance). (2) the hdr_file Authorization header: upstream's side is blank (drops the header), fork keeps -H @\"$hdr_file\" -- unchanged for the same reason as the original decision (dropping it would send the POST unauthenticated). Resolution unchanged; blob bumped.",
