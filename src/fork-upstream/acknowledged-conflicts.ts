...
export const ACKNOWLEDGED_CONFLICTS = {
  ...
  'src/web/hook-registration-guard.ts':
    ...
    " Re-decided 2026-09-25 (backend3, card b5b7eb6b child 577db3a7, 9b3bae69bf..3bdb2fb5ad, +11/-2): two additions, neither adopted, both by the SAME fileExists criterion this rule already applies to itself. (1) Four Slack-counterpart names (slack_progress.py, slack_progress_clear.py, slack_progress_reply_clear.py, slack_progress_watchdog.py) -- verified, none of the four scripts exist anywhere in this checkout, and this fork is Telegram-only per its own root CLAUDE.md ('A Telegram kommunikaciot a Claude Code Channels kezeli'), matching the same out-of-scope call already made elsewhere in this file for other Slack/Discord multi-channel additions. (2) 'memory-frontmatter-gate.py' -- verified, does not exist here either. Since this array only matters for a settings.json entry naming a MISSING file (deciding prunable-ours vs. kept-foreign), adding either name today would be inert either way -- but the established criterion for this exact array is 'add only once the file exists', so neither is added. Resolution (the union-of-both-tails construction) unchanged; blob bumped.",
  ...
  'src/web/context-restart-gate-runner.ts':
    ...
    " Re-decided 2026-09-25 (backend3, card b5b7eb6b child 577db3a7, 83b90ef157..1c25459cd3, +109/-28): the two decided points -- the fork's LOCAL configDirFor (resolveAgentConfigDirForRead, readAgentClaudeConfigDir dropped) and the already-adopted #1202 openQuestionBlocks -- verified UNCHANGED on the fork's actual current file (line-checked directly). Upstream, meanwhile, EXTRACTED configDirFor (plus a new newestMainConfigRoot) out of this file into a brand-new shared module, src/web/main-transcript-root.ts, which does not exist here, specifically to deduplicate a SECOND local copy of the same function that context-guard-runner.ts also carries (verified: that file still has its own separate local configDirFor, line 210) -- AND fixed a real bug in the extracted version: the main agent's OWN isolated config root (a channels session running its own CLAUDE_CONFIG_DIR) was never searched, via mainConfigRoots() from inbound-probe.ts (also absent here, verified zero hits repo-wide). This is the SAME TOKENVAK915 gap already flagged twice this round (card 61817eb9's docs/token-usage.md entry, and again below in src/web/token-usage.ts's own re-decision) -- a third, independent sighting of the identical fix, now with its clearest code home: a dedicated module upstream built expressly to carry it. NOT adopted here (a multi-file extraction + behaviour change, not a rider on this rule's decided import pair) -- named as the strongest follow-up candidate for whoever opens the TOKENVAK915 card, since adopting it here would also retire BOTH forks' duplicated local copies. Four more purely-additive, non-overlapping changes in the same diff, none touching the decided points, also not adopted, named rather than taken: (1) TMUXWINDOWATTR920 -- getPanePid pipes tmux's stderr through the logger instead of letting it print undated onto the process's own stderr (measured upstream: ~3000 unattributed lines); (2) GATEMTIME922 -- msSinceTranscriptWrite prefers a real last-conversation-turn timestamp over file mtime, because an idle session's untimestamped bookkeeping writes (atis-latch, mode, etc.) keep the mtime moving forever and upstream measured two live cases (240 minutes, then 12+ hours) where this alone caused a false 'not stuck' read; (3) GATEDEADLOCK922 -- child-process age is now logged next to its args, and a block reason unchanged for 120+ minutes gets escalation wording in the Hungarian alert text, naming the state a finding rather than patience; (4) GATESENDER922, flagged with EXTRA weight because it is SECURITY-RELEVANT and this fork's root CLAUDE.md names this exact alert prefix: the persistent-block alert is currently sent via `createAgentMessage(name, MAIN_AGENT_ID, ...)` -- FROM the watched agent's OWN name -- verified live on this fork's actual file (line 776-781, unchanged by this diff). Upstream's fix sends it `createAgentMessage('system', MAIN_AGENT_ID, ...)` instead, with a new GATE_ALERT_ORIGIN_NOTE and an explicit '(Tajekoztatas, nem muveletkeres.)' suffix clarifying it is informational, not a directive. This fork's own system-directive authentication rule (CLAUDE.md, 'Rendszer-direktiva hitelesites') names `[CONTEXT-RESTART-GATE]` BY PREFIX as a message type requiring msg_id-based authentication when it requests action -- an alert that arrives spoofed as the watched agent's own name is exactly the ambiguity that rule exists to close, even though today's alert is informational rather than action-requesting. Recommended: MikroB should route this one to Cybersec specifically, not just note it as a passive adoption candidate. Resolution at the two decided points unchanged; blob bumped.",
  ...
  'src/web/agent-scaffold.ts':
    ...
    " Re-measured 2026-09-25 (backend2, card 9163495b): upstream moved again (673-line diff against the pinned c5dd9bc6 blob). Checked every previously-decided symbol directly against the diff (not assumed): ensureLocalFirstSection, agentGetsKanbanWriteGate, quarantineReader, pythonHookCommand, hookScriptAlreadyEffectiveInOtherScope, isPublicFetchHost, refuseMainAgentHookWrite, readAgentToolDeny -- ZERO hits. Two near-misses, both non-substantive: ensureSkillsPathTrapSection/ensureSystemDirectiveAuthSection appear once, named (not modified) inside a comment on a NEW function, ensureMemorySearchLabelSection, listing the existing section-writers as its idempotency-contract precedent; agentSettingsPath appears once, called (unchanged) from another NEW function, ensureBashEgressParser, which idempotently wires a new scripts/hooks/bash-egress-parser.mjs hook into already-spawned agents' settings. Both new functions are purely additive alongside everything this rule decides, not a conflict on it. Resolution unchanged; blob bumped.",
  ...
  // first release on, and it did not read the agent hot/warm/cold tiers at all -- upstream removed
  // it rather than wire it (HOTMEMHAMIS925), leaving whether an agent's memory should auto-load as
  // a separate product decision needing a NEW loader keyed on agent_id+category with a size cap.
  // Verified against THIS fork's current tree, not assumed from upstream's comment: grep for
  // buildMemoryContext across src/**/*.ts finds exactly two things -- its own declaration in
  // src/memory.ts, and its dedicated unit suite src/__tests__/memory-context-truncation.test.ts.
  // No production caller exists here either; the claim upstream's removal comment makes is true of
  // this fork's tree too, independently confirmed rather than taken on faith.
  // Resolution: per kódminőségi elv 3 (pre-existing dead code, unrelated to any change actually
  // being made, gets FLAGGED not deleted on sight) this entry does NOT delete the function as part
  // of a drift-decision round -- that would be applying a behavior change (and an orphaned test
  // file) outside the scope of what this card asks for (recording the merge rule). The RULE for the
  // next real merge: adopt upstream's removal (delete buildMemoryContext() and its MEMORY_CONTENT_MAX_CHARS
  // constant) AND retire src/__tests__/memory-context-truncation.test.ts in the same commit -- do
  // not let the function survive as an even-more-orphaned relic once upstream's side of the
  // conflict marker no longer has it to diff against.
  'src/memory.ts':
    'adopt upstream: delete buildMemoryContext() (zero production callers, verified in this tree too) and MEMORY_CONTENT_MAX_CHARS, retiring its dedicated test src/__tests__/memory-context-truncation.test.ts in the same commit -- not applied in this drift-decision round, only the merge rule is recorded',
  // NEW DECISION 2026-09-25 (card f5536a70, fork/upstream re-decision round). Comment-only conflict,
  // zero code divergence -- both sides document the SAME allowlist invariant (a channel-coordinator
  // device key must ALSO be on the out-of-band voice-channel-device-allowlist.ts, never minted from
  // the shared dashboard token every sub-agent can read). The fork's comment is the fuller version:
  // it names the actual Cybersec NO-GO (card 7503bb31) that forced the allowlist into existence --
  // POST /api/auth/device-keys accepts the shared dashboard token as admin auth, so without the
  // out-of-band allowlist any token holder could self-mint a device key and pass the lane check.
  // Upstream's comment states the same rule without the incident that motivated it. Keeping the
  // fuller comment costs nothing and preserves the "why", matching this file's own established
  // treatment of security-documenting comments elsewhere in this corpus (e.g. src/db.ts's
  // depBlocked/isForceActor note, kept over a terser upstream alternative for the same reason).
  'src/channel-coordinator/ingest.ts':
    'keep the fork\'s fuller comment (names Cybersec NO-GO card 7503bb31 and the exact exploited endpoint) over upstream\'s terser restatement of the same allowlist invariant -- no code divergence, comment-only',
  ...
  'src/web/routes/agents.ts':
    ...
    " Re-measured 2026-09-25 (backend, card 3669d930 landing-block, 44349c36dc6b..6460da896a99, +393/-... one of the largest single increments seen on this file). Zero hits on stopAgentProcess, MiniMax or /api/models/available -- the conflict point and both named exceptions remain untouched. FOUR coherent new capabilities, none of which the fork has (grep-verified: refuseIfCliCannotLaunch/customProvider/writePersonaFileIfUnchanged/contextAvgPerCall all 0 hits): (1) PICKERCLIKAPU923, a CLI-installed-version launch gate on POST/PUT (422 with a Hungarian message when the installed Claude Code CLI cannot launch the requested model); (2) customProvider wiring (readAgentCustomProvider/writeAgentCustomProvider/listCustomProviders) -- the route-level consumer of the same listCustomProviders() already flagged on card 965b0b2b for src/web.ts, a related but distinct file/decision; (3) PERSONANOCLOBBER923, a persona-write-guard so a slow CLAUDE.md/SOUL.md generation never overwrites a file an operator edited by hand while it ran; (4) a measured BUGFIX in the agent model-suggestion signals (contextAvgPerCallMap/kanbanLoadMap) -- the old code read totalInput alone (the uncached remainder, ~2.9 tokens/call against a true ~354k on a long-lived session) and archived_at IS NULL alone for kanban load (counting done-but-unarchived cards as open), both producing wrong-direction model/load advice on the live install, per the diff's own 2026-09-17 measurement note. Not adoptable this round -- four independent decisions, raised together on card 6b10a6b8 (NORMAL), with the bugfix (item 4) flagged there as the lowest-risk, highest-value place to start. Resolution unchanged; blob bumped.",
  ...
  'src/web/routes/kanban.ts':
    ...
    " Re-measured 2026-09-25 (backend, card 3669d930 landing-block, 83ab8067d408..a52b288bb185, +102/-13). Zero hits on the dispatch-text hunk, resolveKanbanDispatch, reportUndeliveredDispatch, self-advance suppression or the /blockers route -- every point this rule decides is untouched. FIVE coherent new capabilities, none of which the fork has (grep-verified: includeArchived/getStuckKanbanCards/parentWouldCycle/kanbanAssigneeExists all 0 hits): a loud-400 unknown-query-param guard on GET /api/kanban plus includeArchived support (the fork silently drops includeArchived today -- measured on the live install: six agents tried three different param names for the same intent, none got feedback); a new GET /api/kanban/stuck endpoint (KANBANSTUCKURES916, a real started-then-idle stuck-card measurement replacing the kanban-audit skill's structurally-near-always-empty inline detector); agent=/assignee= alias with 400-on-unknown-agent (the dashboard and this fork's own CLAUDE.md teach different param names for the same filter); parentWouldCycle (409 on a re-parent that would close a loop, the same reasoning as the existing blockerWouldCycle applied to the parent chain); and an `automated: true` comment flag so a bulk/machine writer's own comment is not read as a work-trace by the stuck detector. Not adopted this round -- five independent decisions, raised together on card 5aaf7209 (NORMAL). Resolution unchanged; blob bumped.",
  // NEW CONFLICT 2026-09-25 (backend, card 3669d930, dashboard/src web/routes 5/10). Upstream's
  // getActiveMarveenModel() adds a configDirFor(MAIN_AGENT_ID) third argument (new module
  // main-transcript-root.js, absent here) to readActiveModelFromProjectDir/readContextTokensFromProjectDir --
  // the fork's own active-model.ts ALREADY accepts that optional configDir parameter, only the caller
  // in this file never passes it. Upstream's own comment: when the channels session runs on its own
  // CLAUDE_CONFIG_DIR, the default root can read a pre-migration transcript that still parses but shows
  // the WRONG model -- display-only, but "a wrong number here is what makes someone trust the wrong root
  // elsewhere". A real, small, measured-motivation fix, but it needs the new main-transcript-root.js
  // module ported first, not a same-round hand-merge. Not adopted this round; raised on card fd8ae23f
  // (NORMAL) together with the overview.ts Fable-quota entry below.
  'src/web/routes/marveen.ts':
    "UNDECIDED, deliberately -- see card fd8ae23f. Fork keeps the async getActiveMarveenModel()/contextTokens shape it already has; the adoption decision is whether/how to thread configDirFor(MAIN_AGENT_ID) through once main-transcript-root.js is ported, not whether to take upstream's sync/async shape wholesale (the fork's Promise.all-based async structure is unrelated to this conflict and stays either way).",
  // NEW CONFLICT 2026-09-25 (backend, card 3669d930, dashboard/src web/routes 5/10). Upstream's
  // overview.ts imports readFableSnapshot + DEFAULT_FABLE_MAX_AGE_SEC from quota.js alongside the
  // existing readQuotaSnapshot/DEFAULT_MAX_AGE_SEC -- grep-confirmed the fork's own quota-snapshot.ts
  // has no readFableSnapshot at all, so Fable-model quota tracking is completely absent from the
  // overview response. Fable is already a live model choice in the picker (claude-fable-5-1, see the
  // agents.ts PICKERCLIKAPU923 entry / card 6b10a6b8), so this is a real, non-trivial gap, not a stale
  // reference to a retired feature. Cross-file (needs the quota-snapshot.ts function ported too), so
  ...
  'src/__tests__/agent-provider-env.test.ts':
    "PARTIALLY ADOPTED 2026-09-26 (backend, card 248d3013). LATENSKULCSARGV920 secret-ref-indirection is now ported: src/web/agent-process.ts's resolveProviderEnv takes a secretShellRef callback instead of a raw secret value, launchSecretRef writes the secret to a private 0600 file and returns a `\"$(cat '<path>')\"` shell reference, and clearLaunchSecrets removes it on stop. Applies today to the deepseek/openrouter/BYO-agent-API-key paths (the ones that exist in the fork). Equivalent tests added here: provider-env-adoption.test.ts's \"KEY never reaches a shell string\" + \"by PROVENANCE\" suites, plus a new launch-secret-ref.test.ts. The custom-provider vault-key-missing-throws half of upstream's file remains unported and stays pending: CustomProviderDef does not exist in the fork at all yet (grep-confirmed zero hits, 2026-09-26) -- that lands as part of card 6b10a6b8 (custom-provider adoption), which is now commented (7123/7124) with the requirement to wire it through this same launchSecretRef/secretShellRef mechanism and to hard-fail on a missing vault key, not fall back silently. Re-check this entry once 6b10a6b8 lands.",
  ...
  'src/web/context-guard-runner.ts':
    ...
    " Re-measured 2026-09-25 (backend2, card 9163495b): upstream moved substantially (234-line diff against the pinned 55de54e0 blob), and unlike agent-scaffold.ts's round this same day, it DOES touch this rule's own decided point, not just its surroundings. configDirFor() -- whose resolver this rule already decided (resolveAgentConfigDirForRead(), SHARPENED 2026-09-03 above) -- is no longer a local function here at all: upstream now imports `configDirFor` from a NEW shared module, ./main-transcript-root.js. This looks like a pure relocation (the same resolver logic extracted for reuse, plausibly for the readTranscriptMtimeAcrossConfigDirs/readLastConversationTsFromProjectDir GATEMTIME922 work the active-model.ts entry above already records) rather than a behavioural change, but main-transcript-root.js's actual contents were NOT read this round to confirm it still calls resolveAgentConfigDirForRead() and not something weaker -- that check is the difference between 'relocated' and 'silently reverted', and skipping it under time pressure is exactly the risk this file's history warns about. measureContextTokens/measureIdleMs/measurePct gained `export` (consumed elsewhere, consistent with the same round), no other visible change to their bodies. A separate, unrelated new feature surrounds them (paneShowsContextSaturationHardError/saturationBannerCredible/noteSaturationBannerUntrusted/clearSaturationBannerOverride -- distinguishing a real context-saturation banner from an untrustworthy pane read) that does not touch what this rule decides. Pin DELIBERATELY LEFT AT THE OLD VALUE, not bumped, until main-transcript-root.js's resolver is read and confirmed against the already-decided resolveAgentConfigDirForRead rule -- deferred to the same supervised cutover (card 5c134edf, F5) this file's other rounds defer to.",
  ...
  'web/app.js':
    ...
    " Re-decided 2026-09-26 (backend, card d87adb90, dashboard/web frontend 10/10, f375962c..04a2f673): +746/-54, seven regions. (1) The X-Memory-Search rescue label (renderMemSearchLabel, loadMemories) is BYTE-IDENTICAL to what the fork already carries (verified: app-memories.js has the same function, same t() keys) -- no gap, already adopted in an earlier round not individually logged here. (2) PICKERCLIKAPU923 (applyClaudeCliGate, openAgentDetail's custom-provider select branch, updateCustomModelIdRow) plus a whole new settings 'Provider-ok' tab (renderProvidersContent) for custom Anthropic-compatible endpoints -- this is the FRONTEND half of card 6b10a6b8's backend decision (agents.ts: CLI-launch gate + custom provider support), genuinely absent here (grep-confirmed: zero hits on applyClaudeCliGate/updateCustomModelIdRow/renderProvidersContent), commented on 6b10a6b8 rather than duplicated as a new card (rule 6b). (3) renderQuotaStrip grew a second `fable` parameter (a Fable/Opus quota row, its own collector and own staleness) -- the FRONTEND half of card fd8ae23f's backend decision (overview.ts: Fable quota snapshot), genuinely absent, commented on fd8ae23f. (4) renderCliUpdateOffer (CLIFRISSAJANLAS923) is a genuinely NEW feature with no existing card -- a Claude Code CLI self-update offer UI reading /api/updates/cli, needing a backend route this fork does not have at all. Raised as its own follow-up (card 6b1020ff is unrelated -- GPU/VRAM test isolation, checked, no overlap -- so this is not a duplicate). (5) The Claude Plans panel (renderClaudePlansPanel/addClaudePlan/loadClaudePlansList) grew substantially: OAuth-token-mode plan auth (vault-stored, never plaintext), a live 5h/7d usage-check button with its own error taxonomy, and a mode selector (token vs CLAUDE_CONFIG_DIR) -- growth on the ALREADY-OPEN card 1c810029 (Claude Plans settings tab port), not a new gap; commented there with the specifics so whoever picks it up ports the grown scope. (6) BRANCH_HEAL_COMMAND (Updates page branch-drift heal instructions) changed from `git checkout main && bash update.sh` to `git switch main || git switch -c main --track origin/main && bash update.sh` -- a self-contained, isolated bugfix (upstream measured: `checkout main` exits 128 on a two-remote repo where git cannot infer which remote's main to follow) with no other feature dependency. ADOPTED DIRECTLY this round in web/app-updates.js (the fork's own copy was still the old, broken form) -- same character as the fleet-helper PYTHONDONTWRITEBYTECODE fix in card 123983f3, no follow-up needed. (7) Zero hits on any previously-decided point (MiniMax, Activity-page deletion, context-guard settings UI, blockers UI, idea-box, thinking-orb) -- all untouched. Resolution at every previously-decided point unchanged; the one direct adopt is (6); blob bumped.",
  ...
  'web/style.css':
    ...
    " Re-decided 2026-09-26 (backend, card d87adb90, dashboard/web frontend 10/10, 4e5fa460..d102534e): +184/-0, purely additive, TWO real conflicts under a live 3-way merge simulation (git merge-file --diff3, base d4f1b1d4). (1) upstream's new `.claude-plan-row*` block collides positionally with the fork's `.cxhu-banner` insertion at the same point (both appended right after `.settings-row-error`) -- pending, pairs with the app.js entry's item (5) (card 1c810029 growth), keep the fork's cxhu-banner as-is. (2) upstream's `.mem-search-label` block ALSO collides positionally, but this one is a NO-OP: the fork already has a byte-identical `.mem-search-label` rule (verified) from its own earlier MEMKERESVAK917 adoption -- both sides converged on the same CSS independently, nothing to take. The remaining additive hunk (`.quota-strip*`) is likewise already present verbatim in the fork (card c6d8d599) -- no gap, though note a PRE-EXISTING, unrelated fork-only duplication found while checking this: `.quota-strip` and its sibling rules appear TWICE in the fork's own style.css (once with the c6d8d599 attribution comment, once unattributed right after a `.usage-llm-info-grid` media query) -- flagged, not fixed here (surgical-changes principle: this diff is about the upstream comparison, not fork-only dead-code cleanup; worth a trivial follow-up whenever this file is next touched). Resolution unchanged; blob bumped.",
  ...
  'src/web/message-router.ts':
    ...
    " Re-decided 2026-09-25 (backend3, card b5b7eb6b child 577db3a7, f962c8be87..178c5f988c, large additive growth): both decided points verified UNCHANGED on the fork's live file (getKanbanCardStateByIdPrefix + formatDeliveryStalenessNote at line 782/1064; countNewerMessagesFromSameSender + freshness at line 1057-1058, both present, zero symbol collision). Upstream added several independent, non-overlapping features, none touching either decided point (verified against each new symbol, zero hits on getKanbanCardStateByIdPrefix/formatDeliveryStalenessNote/countNewerMessagesFromSameSender in the new hunks): (1) a permission-prompt-aware stuck-session alert (detectsPermissionDialog/permissionPromptSummary) that stops a tool waiting on a yes/no answer from being misread as a wedged session and restarted -- upstream's own comment cites a near-miss on 2026-09-03 where ~165k tokens of running sub-agent work was one step from being discarded this way; (2) selectTickWindow for fair per-recipient tick-window selection, replacing a plain oldest-first slice that let one busy recipient's backlog starve every other recipient's turn; (3) a getMessageStatus liveness re-check immediately before send, skipping (not re-closing) a row that went non-pending during the tick's own gap; (4) multi-envelope batch injection (B1F38C8C) for congested recipients, opt-in per agent. All four are genuine reliability/safety improvements with no overlap with what this rule decides -- not adopted here (each is a multi-part behaviour change), named as follow-up candidates, (1) and (3) the strongest of the four on a quick read. Resolution unchanged; blob bumped.",
  ...
  'src/web/token-usage.ts':
    ...
    " Re-decided 2026-09-25 (backend3, card b5b7eb6b child 577db3a7, 82ebcf785c..7b097efb62): the two prior decided points (all-four-imports-together; correlateWithKanban's AND NOT EXISTS parent-skip filter) verified UNCHANGED (estimateCostUsd/stripDateSuffix still imported and used at line 490, correlateWithKanban untouched by this diff). The FOURTH round of the still-open idx_token_usage_dedup agent-first-column bug is also untouched here -- still its own card, not this diff's subject. What upstream DID land this round is the CODE-SIDE fix for TOKENVAK915, in the clearest form yet, IN THIS EXACT FILE: discoverAgentSources() now reuses mainConfigRoots() (from inbound-probe.js, absent here) to also search the main agent's OWN isolated config roots (a channels session running its own CLAUDE_CONFIG_DIR), deduping by realpath against what the existing sub-agent loop already found. Upstream's own comment documents the measured failure this closes, on their install: the newest transcript under the default root was frozen 30 hours stale while the live file sat under a different, unsearched root, and the dashboard kept showing a number instead of a gap -- the identical shape flagged from the doc side in card 61817eb9's docs/token-usage.md entry, and independently again in schedule-runner.ts above (same mainConfigRoots() reuse). Three sightings of the same fix in one round is a strong signal for whoever opens the follow-up card. Two smaller, independent, non-overlapping changes in the same diff: (1) the fork's own LOCAL encodeProjectPath() duplicates what upstream extracted into a new shared src/claude-project-dir.ts (encodeClaudeProjectDir) -- not created here, a safe future dedup, zero behaviour difference measured (same regex); (2) a new getModelSourceBreakdown() diagnostic export (APRO920/ELSOKOR922) breaking a model's token rows down by agent/session/task, plus a toolInputPreview() fallback so a tool-calling turn with no text block no longer logs an empty preview -- both pure additions, no fork equivalent, no conflict surface. None of this touches the two decided points or the still-open dedup-key card. Not adopted here (TOKENVAK915 is a behaviour change deserving its own gate, same reasoning as the other two sightings; the two smaller items are cosmetic/diagnostic, cheap to pick up later but not urgent). Resolution unchanged; blob bumped.",
  ...
  'src/web/schedule-runner.ts':
    ...
    " Re-decided 2026-09-25 (backend3, card b5b7eb6b child 577db3a7, e5393ace22..058bb70dc8): both decided points verified UNCHANGED on the fork's live file -- runPreCheck stays a bare `export function` with no `async` on upstream's own new blob (line-checked directly, upstream itself is still sync at this line), and zero hits on startAgentProcess anywhere in the diff. A large batch of new upstream features, none overlapping: heartbeat-metrics injection into scheduled-task prompts (collectHeartbeatMetricsBlock), reference-based large-prompt delivery for oversized scheduled tasks (wrapScheduledTaskByReference + scheduled-run-snapshot.js, with new SCHEDULED_TASK_INLINE_MAX_CHARS/BODY_WARN_CHARS/MAX_PROMPT_LEN thresholds), a per-process dedup guard for an ambiguous-recipient config-gap alert (so a mis-pinned task does not re-alert on every fire), and delivery-integrity verification (classifyDelivery/readUserPromptsSince) reading the pane and recent prompts to confirm a scheduled task actually reached the target session. Also present: `import { mainConfigRoots } from './inbound-probe.js'` and `readTranscriptMtimeAcrossConfigDirs` -- a THIRD independent site carrying the same TOKENVAK915 main-agent-isolated-config-root fix already flagged twice this round (card 61817eb9's docs/token-usage.md entry, and src/web/context-restart-gate-runner.ts above), strengthening rather than changing that follow-up's case. None of this touches runPreCheck's signature or the startAgentProcess try/catch. Not adopted here (each is its own multi-part feature); the delivery-integrity verification is worth flagging as a strong candidate given how much of this file already exists to catch silent non-delivery. Resolution unchanged; blob bumped.",
  ...
  'src/web/agent-process.ts':
    ...
    " Re-measured 2026-09-25 (backend2, card 9163495b): upstream moved substantially (1081-line diff against the pinned 46155ebb blob), and THIS round has real hits on the rule's own decided points, unlike several rounds before it. (1) resolveProviderEnv() gained a NEW secret-handling layer since the 2026-09-04 byte-identical adoption above: LAUNCH_SECRETS_DIR_MODE=0o700 plus a launchSecretRef()-based indirection (comment names it LATENSKULCSARGV920) -- the launch command now carries a REFERENCE to a secret file instead of the raw value inline, so a provider key (BYO/deepseek/minimax/openrouter) never lands in argv/`ps`, the same threat class this file's own oauthTokenEnv comment already names for the OAuth token. This reads as a genuine, valuable hardening in the SAME category the fork already adopted resolveProviderEnv for -- but it changes the function's call contract (the secretLookup callback now returns a file reference, not the raw value) and was NOT measured byte-for-byte the way the 2026-09-04 adoption was (provider-env-adoption.test.ts) before this note was written. ProviderKind also widened with a new 'custom' entry, tying to the custom-provider launch-env feature the agent-worker.ts entry above records as upstream-only. NOT adopted this round: a credential-handling code path used on every agent launch is exactly the kind of change this file's history says not to hand-merge without the same discipline the 2026-09-04 round used. (2) umask 002 + agentTmuxTarget(name)/startTarget is STILL present upstream (now also reached from a NEW resumed-channel-agent relaunch path) and STILL not adoptable for the SAME reason as 2026-09-04 -- no runAsUser consumer exists in this fork; this diff's added lines carry no readAgentRunAsUser/sessionRunAsUserMap/sudoers-rule symbols, consistent with that absence, though a full re-count was not repeated this round. (3) The agent-scaffold import-line conflict is now purely MECHANICAL: upstream added several new, unrelated section-writer imports (ensureProjectRootInClaudeMd, ensureMemorySearchLabelSection, ensureFleetAuthSection, ensureEvidenceSection, ensureMcpListChannelSection -- all part of the agent-scaffold.ts entry's own 2026-09-25 additive round) on the same import line as the already-decided ensureSystemDirectiveAuthSection/ensureSkillsPathTrapSection names, not a change to what this rule already unions there. Because (1) is a real, unreviewed, security-relevant change to a launch-time path, the pin is DELIBERATELY LEFT AT THE OLD VALUE rather than bumped -- bumping would assert a review this round did not perform to the standard the prior rounds set. Full reconciliation, including a byte-identical-output measurement for the new secret-ref layer, deferred to the same supervised cutover (card 5c134edf, F5) this file has deferred every substantial round to.",
  ...
  // Card b5b7eb6b child 61817eb9 (2026-09-25, backend3): five docs/templates/skills/CI files arrived
  // as UNWATCHED conflicts. Verified with a real 3-way merge simulation for the conflict boundary,
  // AND (per the qa2 lesson on 09d54e88 above) a plain two-dot `git diff HEAD <upstream>` for each
  // file to make sure a large raw diff was not hiding a second, non-conflicting change -- for a
  // NEVER-PINNED file the two-dot diff also includes ALL long-standing structural divergence (e.g.
  // templates/CLAUDE.md.template's 176-line "Munkavégzési szabályok" section, which upstream has
  // never had at all, not a new removal to decide on), so the merge's OWN conflict markers, not the
  // two-dot diff's raw line count, are what actually need a decision for a file at this stage.
  '.github/workflows/test.yml':
    "KEEP the fork's line (bash scripts/__tests__/ollama-boot-restore.test.sh) -- upstream's side adds NINE more CI steps (egress-drift-scan, dash-audit-scan, mio-orszem-precheck, memory-link-audit, keepalive-probe-install, morning-stamp-gate, morning-timer-park, userbot-arming-gate, supabase-q-token-hiding), and ALL NINE reference scripts that do not exist anywhere in this fork (verified: `find scripts/__tests__` for each name, zero hits) -- adopting upstream wholesale would break CI outright on every push, referencing files that were never ported. Each named script belongs to an upstream feature this fork has not adopted (community sentinel, dash audit, keepalive probe, morning briefing timers, userbot arming, supabase token hiding) -- an adoption question for whoever wants any of those features, not a CI-wiring decision.",
  'docs/telegram-progress-indicator.md':
    "ADOPT upstream in full (both hunks: the provider-gate explanation paragraph, and the 'Remove' section's `bash scripts/retire-progress-watchdog.sh telegram --force` in place of raw launchctl/systemctl commands) -- paired with scripts/install-telegram-progress-hook.sh's own ADOPT decision above (card 09d54e88 child 1/10): the doc must describe what the code now actually does, and the code side already adopted the provider-gate + retire_provider() mechanism.",
  'docs/token-usage.md':
    "NEITHER side verbatim -- both are stale against the ACTUAL current code, which the drift report's two-hunk framing (HEAD vs upstream) does not surface as an option. Hunk 1 (agent discovery): KEEP the fork's text (three-source discovery via mainConfigRoots() does not exist anywhere in src/ -- verified, zero hits) but flag upstream's third source as a REAL, worth-adopting bug fix (TOKENVAK915: the main agent's own isolated config root was never searched, so its token rows silently stopped appearing for 30 hours while the dashboard kept showing a stale-but-parseable number) -- a code change, not a doc-conflict resolution, so raised as its own follow-up rather than taken here. Hunk 2 (dedup): NEITHER side is current -- checked against the ACTUAL schema (src/db.ts:1427) and the ACTUAL insert (src/web/token-usage.ts:332-339): the unique index has ALREADY had `agent` dropped from its key (card b774f057, landed) and the INSERT already uses ON CONFLICT...DO UPDATE filling model/thinking_tokens on NULL -- exactly what upstream's hunk describes, except upstream's text still lists `agent` as part of the key, which is no longer true here. The fork's OWN hunk text is even more stale, describing the migration as 'still planned and blocked'. Neither hunk is adoptable as-is; the doc needs fresh text matching the current schema, which is a documentation-accuracy fix outside this drift re-decision's scope -- flagged, not written here, since getting the exact current index/upsert shape right belongs with whoever touches this file next for a real reason, not backfilled blind. Separately, src/web/token-usage.ts's OWN comment (lines 34-40) has the same staleness (says b774f057 'is still planned and blocked') -- a source-comment fix, smaller than a doc rewrite, also flagged rather than fixed here to keep this round to conflict resolutions.",
  'seed-scheduled-tasks/kanban-audit/SKILL.md':
    "KEEP the fork's inline-python client-side counting for BOTH hunks (archiving-candidate count, stuck/waiting detection) -- upstream's replacement calls a dedicated `/api/kanban/stuck?planned_days=N&active_days=N` server endpoint with a structured JSON response (examined/stuck/waiting/overdue fields, so a zero result can be told apart from 'could not measure' -- the same measurement-honesty principle this fork already applies elsewhere). Verified: no `/api/kanban/stuck` route exists anywhere in src/web/routes/kanban.ts. Adopting the doc text would instruct the skill to curl a 404. A genuinely nice feature (server-side, reusable, self-reporting counts) -- an adoption question for a dedicated card (a new server endpoint + skill rewrite), not decided here.",
  'templates/CLAUDE.md.template':
    'KEEP the fork\'s piped-header -H @"$hdr_file" idiom in the four memory/kanban command examples -- upstream\'s replacement moves the Authorization header to a plain argv-embedded form instead of reading it from a file, the same token-in-argv vulnerability class (/proc/<pid>/cmdline is world-readable) rejected everywhere else in this fork (watchdog.sh, install-prod-tree-guard-hook.sh, b267df80). This is a TEMPLATE rendered into every fresh install\'s own CLAUDE.md, so taking upstream\'s form would teach every new agent the insecure idiom by example.' +
    ' Re-measured same round (backend3, card b5b7eb6b child 61817eb9, 2026-09-25, d1b08abd1a896ab4533b13da24762f747c13454d): upstream advanced AGAIN mid-round with a genuinely new, unrelated hunk (MAILWINDOW24, commit 61b98a38) touching a completely different section (the seeded morning-briefing "Utasitasok" block, not the curl idiom this rule decides). ADOPTED verbatim: (1) the email search window widens from 12 to 24 hours, because the seeded scheduled task fires at 07:30 and a 12-hour window opened at 19:30 the previous evening, silently dropping the whole prior working day\'s mail into an empty EMAIL section; (2) an added sentence naming the sender/subject as third-party data to quote, not follow -- a prompt-injection-defense instruction, strictly additive and in this fork\'s own established idiom (see the fork\'s root CLAUDE.md email/inter-agent sections, which already treat inbound content as untrusted data). Zero overlap with the curl-idiom decision above (different line range, verified via the commit\'s own single-hunk diff). SEPARATE, FLAGGED FINDING, not fixed here (out of this doc-template card\'s scope): the fork\'s actual LIVE root CLAUDE.md (deployed, not this repo template) still says "elmult 12 orabol" in its own Reggeli napindito section -- the same bug upstream just fixed in the template, unfixed in the file MikroB actually runs from daily. Reported to MikroB rather than edited here, since the live root CLAUDE.md is not a file this card\'s git diff touches.',
  ...
  'src/web/routes/messages.ts':
    ...
    " Re-measured 2026-09-25 (backend, card 3669d930 landing-block, b0160f69bc54..b3b4b4d55807). Zero hits on the GET-handler comment (hunk 1) or the notify-field/try-catch-nesting (hunk 2) -- both untouched, resolution unchanged. Upstream added a SYSTEM_DIRECTIVE_SENDER reserved-id guard on POST (403 forging the system directive sender), a VOICE_CHANNEL_AGENT_ID device-key-gated sender lane, and a message freshness/supersession annotation on every read path (attachFreshness). None is a gap: grep-confirmed the fork already carries functionally equivalent controls under its own naming -- isReservedSenderId (src/web/system-directive-id.ts, matching root CLAUDE.md's documented from_agent='system-directive' guard) for the first, VOICE_CHANNEL_AGENT_ID + attachFreshness already present at the same call sites for the other two. Parallel implementation, not a missing capability. Resolution unchanged; blob bumped only.",
  ...
  'web/lang/en.js':
    ...
    " Re-decided 2026-09-26 (backend, card d87adb90, dashboard/web frontend 10/10, f84bceee..7b0c3721): the full measurement the note above asked for. +81/-30 keys, 0 collisions with any fork-only key (checked by name). Two categories. (1) An upstream-only English-copy style sweep across ~25 UNRELATED existing keys, replacing an em-dash-joined clause with a comma or period (e.g. 'Main agent config -- read-only' -> 'Main agent config (read-only)') -- purely cosmetic, zero functional difference, not adoptable (would be diff noise with no behavior change, kódminőségi elv 2/3). (2) New keys, all pairing with genuinely-absent features already tracked elsewhere in this file's web/app.js entry: overview.quota.fable (Fable quota row, card fd8ae23f), agents.model.fable51/opus55_1m/cliUnsupported/cliUnmeasured (CLI-launch gate + updated model list, card 6b10a6b8), updates.cli.* (CLI-update-offer, new follow-up), settings.claude_plans.token_mode/status.*/checked/check_now/checking/check_needs_token/probe_*/form.mode*/form.token* (Claude Plans token-mode growth, card 1c810029), memories.relaxed.title/body (ALREADY PRESENT verbatim in the fork -- own MEMKERESVAK917 adoption, no gap, just a naming coincidence with upstream's later addition of the same pair). None of the new keys are adopted here (inert without their JS/route counterparts, same posture as every prior round's key-only additions); union rule holds regardless -- keeping unused upstream keys costs nothing and avoids re-deriving this measurement when the paired features eventually land. Resolution unchanged; blob bumped.",
  'web/lang/hu.js':
    ...
    " Re-decided 2026-09-26 (backend, card d87adb90, dashboard/web frontend 10/10, 8d0ec7dd..2bb4f599): identical shape to en.js this round too (+81/-30 keys, 0 collisions, same em-dash-style-sweep + Fable-quota/CLI-launch-gate/CLI-update-offer/Claude-Plans-token-mode/memories.relaxed key set, verified by NAME to match en.js's addition exactly -- lockstep holds). Resolution unchanged; blob bumped.",
  ...
  'web/index.html':
    ...
    " Re-decided 2026-09-26 (backend, card d87adb90, dashboard/web frontend 10/10, 014925c8..0b5d14a3): +34/-4, no overlap with the MiniMax point above (checked). Live 3-way merge simulation (base d4f1b1d4): one real conflict, everything else auto-merges clean. The quota-strip and mem-search-label markup upstream adds are ALREADY PRESENT verbatim in the fork's live file (cards c6d8d599 and MEMKERESVAK917) -- no gap. The updated Claude model list (claude-fable-5-1, claude-opus-5-5[1m]) plus the two new custom-provider optgroups/inputs/CLI-hint paragraphs auto-merge cleanly onto the fork's unmodified model-select markup -- this is the frontend half of card 6b10a6b8's backend decision, genuinely absent, commented there (same pairing as web/app.js's item (2)). The ONE real conflict: upstream's `updatesCli` div insertion point (right before the updates-commits heading) collides positionally with the fork's own restructured 'Beépített repók' page insertion at that same spot -- purely structural, not a logical disagreement, and pending anyway (the CLI-update-offer feature has no existing card, raised fresh on web/app.js's item (4)). Resolution unchanged (keep fork side on the decided MiniMax point); blob bumped.",
  ...
  'src/__tests__/hook-registration-completeness.test.ts':
    ...
    " SUPERSEDED 2026-09-25 (backend, card f5536a70, fork/upstream re-decision round; 78bead60..5c5f73f2): the EXEMPT-list framing above no longer describes the shape of upstream's file at all, so the 2-replacement-entries comparison it made is moot -- not because the answer changed, but because the question did. Upstream (TGSABLONHOOK921, 2026-09-21) restructured the whole mechanism: it now splits REGISTRATION_SURFACES into SEEDING_SURFACES (every future agent gets these: template, agent-scaffold, the two installer scripts, now also install-slack-progress-hook.sh) and CHECKOUT_SURFACES (only the main agent, via .claude/settings.json), plus a scheduled-tasks corpus that now also reads task-config.json (a preCheck script counts as a registration too). A new CHECKOUT_ONLY map lists hooks the checkout wires on purpose that must NOT be seeded (email-approval-gate.py, inbox-drain.py, the ledger-capture/outbound/replay trio, telegram-reply-guard.py -- each with a sourced, main-agent-only reason), and a new exported unseededCheckoutHooks() plus three new test cases enforce that anything wired ONLY in the checkout's own settings.json is either seeded somewhere or has a reasoned CHECKOUT_ONLY entry. WHY THIS IS REAL, not cosmetic: it closes a measured blind spot -- a hook present in the checkout's own .claude/settings.json used to satisfy the OLD single-corpus lint by itself, so a hook that was never added to the SEEDING surfaces (template/agent-scaffold/installers) stayed green while every newly-seeded agent silently started without it (upstream's own worked example: telegram-reply-directive.py, measured missing on two live agents across two separate incidents). This fork's CURRENT file is still the OLD single-corpus shape (verified: zero hits on SEEDING_SURFACES/CHECKOUT_SURFACES/CHECKOUT_ONLY/unseededCheckoutHooks in the tree) -- meaning the SAME blind spot this upstream round closes is open here too, unmeasured. NOT resolved in this drift-decision round: adopting it correctly requires walking this fork's OWN .claude/settings.json against its OWN seeding surfaces to build a fork-specific CHECKOUT_ONLY list (the fork's checkout-only hooks are not the same set as upstream's -- e.g. this fork's ensureSystemDirectiveAuthSection/pythonHookCommand-era additions are not in upstream's CHECKOUT_ONLY at all), which is an adoption decision, not a passive conflict resolution. Raised on follow-up card e344e366 (TGSABLONHOOK921 hook-seeding split). Blob bumped to record the measurement; the 14-entry EXEMPT-list rule stays as the CURRENT-file's actual behavior until that follow-up lands and replaces the mechanism.",
  ...
  // Card 9163495b (2026-09-25, backend2), fork/upstream re-decision, area "dashboard/src --
  // agent-eletciklus" (parent b5b7eb6b). The 5 entries below were previously undecided.

  // Both readActiveModelFromProjectDir and readContextTokensFromProjectDir carry a measured,
  // two-round fix (cards d3dc35bf, 9a2fd3f7): the naive whole-file readFileSync on GET /api/agents's
  // request path put a SYNCHRONOUS read of up to 276 MB on the JS thread, measured 3.8-7.4s (p50
  // 4.4s) with every OTHER route stalling behind it -- the fleet's own agents reach each other
  // through this API, so a slow read here froze the whole fleet. The fix: a bounded tail read (the
  // last N bytes, widened once if empty) PLUS moving it onto fs/promises so libuv's threadpool does
  // the blocking part instead of the JS thread PLUS coalescing concurrent callers for the same key
  // into one in-flight read (a 20-request burst against 6 agents went from freezing the loop to
  // 0.21-4.4s). Upstream's current file reverted all three: synchronous, whole-file, no coalescing.
  // Adopting it would reopen the exact incident the fork measured and fixed -- not taken.
  'src/web/active-model.ts':
    "KEEP the fork's async + bounded-tail-window + request-coalescing readActiveModelFromProjectDir/readContextTokensFromProjectDir (cards d3dc35bf, 9a2fd3f7) -- upstream's current synchronous whole-file readFileSync is a measured regression (GET /api/agents froze the whole fleet, 3.8-7.4s stalls). ADOPT upstream's two genuinely new, unrelated additions on top of the kept fork functions: (a) encodeClaudeProjectDir from the new src/claude-project-dir.ts (measured correctness fix -- the fork's naive `.replace(/[/.]/g, '-')` in projectsDirFor silently diverges from Claude Code's real encoding on paths with underscores/spaces/accented characters, which then read as \"no value\" for context counter/active model/etc; create src/claude-project-dir.ts verbatim, zero-dependency pure function, and use it in projectsDirFor); (b) the new readLastConversationTsFromProjectDir + readTranscriptMtimeAcrossConfigDirs (card GATEMTIME922 -- transcript mtime keeps climbing from timestamp-less bookkeeping writes while a session sits idle, which blocked the context-guard restart gate for 240 minutes with no stuck work; this reads a WIDENING bounded tail capped at a 64MB full-read fallback, so it does not reopen the sync-whole-file-block issue the kept functions already fixed). Neither addition touches or depends on the async/coalescing machinery being kept.",

  // Security-critical: KEEP wins outright here, nothing from upstream's structural side is adopted.
  'src/web/agent-config.ts':
    "KEEP the fork's fail-closed listAllAgentNames() (filters isReservedSenderId) + listRejectedAgentDirNames() + AgentDirRejection[Reason] types (card 53c59307, Cybered's finding) -- without the filter, any live agent can `mkdir agents/system-directive` with plain Bash and join the context-guard sweep, whose writers send messages carrying the swept directory name as `from`, minting a reserved sender id nothing else in the fleet can mint. Upstream's current listAllAgentNames() is the unfiltered rename of the old agentDirNames() -- a straight regression of this fix, not adopted. Also KEEP the fork's MODEL_ID_RE/isValidModelId/InvalidModelIdError re-export from model-id.js (load-bearing: model-fallback-runner.ts and routes/agents.ts import these symbols from HERE, not from model-id.js directly -- card d041760b) and knownModelSuggestions() (fork-only dashboard feature, card 5d2002b5, no upstream equivalent to reconcile against). Do NOT take upstream's MODEL_ALIASES.opus value ('claude-opus-5[1m]') -- card d041760b/Peti 2026-08-06 pins the bare 'opus' alias to DEFAULT_MODEL_CHAIN[0] and requires the two move together; whether the ladder primary becomes a 1M-context variant is its own decision, not a merge byproduct. ADOPT (additive, nothing fork-side lost): upstream's sanitizeToolDenyList()/TOOL_DENY_NAME_RE/TOOL_DENY_MAX_PER_AGENT wired into readAgentToolDeny() (strengthens the existing card-21597530 deny-list handling -- still additive-only per that card's own argument, now also pattern-checked and bounded) and the new readAgentCustomProvider/writeAgentCustomProvider (unrelated new per-agent config, no naming or behaviour collision with anything fork-specific). This file sits on the same security-critical agent-lifecycle surface as agent-process.ts/agent-scaffold.ts below (179-line diff) -- actual code merge deferred to the same supervised cutover (card 5c134edf, F5); this entry records the decision for when that happens, it is not itself the merge.",

  // Same tmux-resolver class the block-opening src/web/routes/agent-terminal.ts entry above
  // documents: upstream routes tmux calls through tmuxInvocationFor (agent-process.ts) to support an
  // agent owning its own OS user/tmux server, whose reconciliation is itself deferred (card e80c011a,
  // 22 hunks on a 1600-line security-critical file). THIS file's upstream copy has not gone that far
  // yet -- it only swapped the fork's lazy tmuxBin() for an eager `resolveFromPath('tmux')` -- so the
  // same interim logic applies: the lazy resolver only throws on first actual tmux use, the eager one
  // throws at import time on a host without tmux on PATH yet.
  'src/web/agent-worker.ts':
    "KEEP the fork's tmuxBin() lazy resolver and its explicit MARVEEN_WORKER_MODEL/.env WORKER_MODEL fallback ordering over upstream's eager resolveFromPath('tmux') -- same interim-vs-full tmuxInvocationFor reasoning as the routes/agent-terminal.ts entry above, pending the agent-process.ts cutover (card e80c011a). ADOPT (additive, unrelated): the new tmuxStderr()-based logging on the kill-session catch branch (piped stderr instead of a silently swallowed exception, card TMUXWINDOWATTR920) is safe to take standalone. Upstream's custom-provider launch-env support (buildCustomProviderLaunchEnv/stampCustomApiKeyApproval imported from agent-process.ts, workerModelSource()) is genuinely new functionality, not a conflict resolution -- it lets a Claude-subscription-less fleet (custom LiteLLM endpoint) get working background workers by inheriting the main agent's custom provider. Its exports currently only exist in upstream's copy of agent-process.ts, so its actual adoption is gated on that same file's supervised cutover (card 5c134edf, F5); recorded here so the decision travels with this file once that lands.",
...
export const ACKNOWLEDGED_UPSTREAM_BLOBS: Readonly<Record<keyof typeof ACKNOWLEDGED_CONFLICTS, string>> = {
  ...
  'src/web/hook-registration-guard.ts': '3bdb2fb5ad84325a9cf9c9dcd9edfeace526346f',
  ...
  'src/web/context-restart-gate-runner.ts': '1c25459cd330c5dd58e5107dad0ff90f0e7001e4',
  ...
  'src/channel-coordinator/ingest.ts': 'fc13ad6f84fa90c86194d505dc05279275301b8c',
  ...
  'src/web/routes/kanban.ts': 'a52b288bb185778148b304e713bcfc86f01a7621',
  ...
  // playwright stage.
  'web/app.js': '04a2f673f1603d8e410edc569299ce462503bd68',
  'web/style.css': 'd102534e9b0e259791b623e0ff020c2066d37679',
  ...
  'src/web/message-router.ts': '178c5f988c61fa0506aaf84b7b53219ee62db951',
  'scripts/start.sh': '67edd340c70b91662f3a057c42686c53b7ca7c1f',
  'src/web/token-usage.ts': '7b097efb62193c347c4a565f23341551cb8db6d0',
  ...
  'src/web/schedule-runner.ts': '058bb70dc871984f0c8c3ea7363cb08807cb7dd5',
  ...
  'src/__tests__/notify-delivery-honesty.test.ts': 'f84e69aad4a3ebad926004d168446a5394c52d78',
  ...
  // Card b5b7eb6b child 61817eb9 (2026-09-25, backend3): five new pins above.
  '.github/workflows/test.yml': '2970438ae4ca0491e3c4984cc2fd39771862a039',
  'docs/telegram-progress-indicator.md': '49b6a89aa365836b0d288a1123347eade4a5bf06',
  'docs/token-usage.md': 'e7c9ae8426e9a62372d9aeffbedbd5cb2110d4a9',
  'seed-scheduled-tasks/kanban-audit/SKILL.md': '60ae0971e075db7b8e26d208d87ac4b6012c6d8b',
  'templates/CLAUDE.md.template': 'd1b08abd1a896ab4533b13da24762f747c13454d',
  ...
  'web/lang/en.js': '7b0c37211400bcf4fb378d9500dfb9eeeaf7e944',
  'web/lang/hu.js': '2bb4f59929404a8b96856e32ae79d43e9a2a7ce1',
  ...
  'src/web/routes/memories.ts': 'c25e5264084fb4e79e1bcd6f6cbc984964e07971',
  'templates/sub-agents/quarantine-reader.md': '22bbd9e4e0e134ba91b95a8cb170b546774f0884',
  'web/index.html': '0b5d14a391cc65689c62dffbf61a3ec1d21a7677',
  ...
  'src/__tests__/hook-registration-completeness.test.ts': '5c5f73f28afdcd71d181bd045b5bd40b36e1a764',
  ...
  // Card 9163495b (backend2, 2026-09-25), fork/upstream re-decision, area "dashboard/src --
  // agent-eletciklus" -- see the matching ACKNOWLEDGED_CONFLICTS entries above for the reasoning.
  'src/web/active-model.ts': '119f10e7b2155e1874123b9bb7c45265fb5555a2',
...
export const ACKNOWLEDGED_FORK_ANCHORS: Partial<Record<keyof typeof ACKNOWLEDGED_CONFLICTS, ForkAnchor>> = {
  // Card 9163495b (backend2, 2026-09-25): agent-config.ts's entry above refuses upstream's
  // unfiltered listAllAgentNames() -- keeping the fork's fail-closed version (card 53c59307) means
  // listRejectedAgentDirNames() and the isReservedSenderId filter it documents stay in this file.
  // The fork-only function is the checkable fact: it has no upstream counterpart at all, so its
  // disappearance can only mean the fail-closed filtering was dropped (by a hand-merge or a history
  // merge quietly taking upstream's plain rename), not that upstream caught up to it.
  'src/web/agent-config.ts': {
    needle: 'export function listRejectedAgentDirNames',
    file: 'src/web/agent-config.ts',
    expect: 'present',
    because:
      "card 53c59307's fail-closed listAllAgentNames() (filters isReservedSenderId, refusing " +
      "upstream's unfiltered rename) is what stops any live agent from mkdir'ing agents/system-" +
      "directive and joining the context-guard sweep under a reserved sender id. " +
  ...
  'src/web/message-router.ts': {
    needle: 'selectTickWindow',
    file: 'src/web/message-router.ts',
    expect: 'absent',
    because:
      "Card b5b7eb6b child 577db3a7 (2026-09-25): NOT adopted -- upstream's fair per-recipient tick-" +
      "window selection (replacing a plain oldest-first slice that let one busy recipient's backlog " +
      "starve every other recipient), along with three sibling features from the same round " +
      "(detectsPermissionDialog/permissionPromptSummary, a getMessageStatus liveness re-check before " +
      "send, and multi-envelope batch injection) -- none touch this file's decided import-pair union. " +
      "If selectTickWindow (or the './message-router-window.js' import it lives in) ever appears here " +
      "without a fresh decision, that is a silent adoption via history-merge, not a merge resolution.",
  },
  'src/web/agent-scaffold.ts': {
  ...
  '.github/workflows/test.yml': {
    ...
    expect: 'absent',
    because:
      "Card b5b7eb6b child 61817eb9 (2026-09-25): NOT adopted -- upstream added nine CI steps for " +
      "scripts this fork does not have (egress-drift-scan, dash-audit-scan, mio-orszem-precheck, " +
      "memory-link-audit, keepalive-probe-install, morning-stamp-gate, morning-timer-park, " +
      "userbot-arming-gate, supabase-q-token-hiding -- verified zero hits for any of the nine under " +
      "scripts/__tests__/). This one is the representative needle: if it ever appears in this " +
      "workflow, someone wired in at least the egress-drift-scan step, which means the underlying " +
      "community-sentinel feature (and likely some of the other eight) landed too -- re-decide the " +
      "whole batch then, not just this one line.",
  },
