...
export async function gatherGateInputs(name: string, nowMs: number): Promise<GateSnapshot> {
  ...
  const contextTokens = await readContextTokensFromProjectDir(workingDir, configDirFor(name))
 ...
 */
export async function diagnoseAgent(name: string, nowMs: number) {
  const snapshot = await gatherGateInputs(name, nowMs)
...
async function checkAgent(name: string, nowMs: number): Promise<void> {
  ...
  const { cfg, session, mcpPatterns, inputs, dispatchedStatsFailed } = await gatherGateInputs(name, nowMs)
