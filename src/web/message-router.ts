...
export async function runMessageRouterTick(): Promise<void> {
    ...
    for (const msg of pending) {
      ...
      try {
        ...
        // MERGE UNION (B-wave, card 42938a74): upstream routes a worksource agent to its QUEUE
        // instead of the keyboard; the fork appends two router-side notes to the tmux payload.
        // Both kept. The notes stay on the tmux path only -- upstream's queue reader formats the
        // item itself, and inventing a note format for a path I cannot exercise here would be a
        // guess. Raised as a named follow-up instead: the staleness/backlog facts are just as
        // true for a queued hand-off.
