...
import { writeAgentSettingsFromProfile, ensureFleetRosterSection, ensureAutonomySection, ensureLocalFirstSection, ensureSkillsPathTrapSection, ensureSystemDirectiveAuthSection } from './agent-scaffold.js'
...
// NO RESEND_API_KEY HERE, DELIBERATELY (card 691f5475 / 0ea08957, after incident f8db701c).
//
// Every launcher used to export that variable from the 0600 key file at launch, which put the
// live credential in the environment of all 15 sessions for their whole lifetime. (The removed
// shape is deliberately NOT quoted here: the guard in resend-key-env.test.ts matches the shape,
// not the name, and a comment that spells it out reads as the thing itself -- card 48d5f255. It
// caught this very comment first time round.)
//
// Reading it via $(cat) kept it out of argv, but not out of `env` -- and that is how it leaked: an agent pasted a
// full environment dump into an inter-agent message, publishing the key to every recipient's
// transcript. An ambient secret only needs ONE quoting slip anywhere in the fleet.
//
// The `resend` MCP server now resolves its own credential at connection time through
// scripts/vault-headers-helper.sh (`headersHelper`), so no session needs the value at all.
// Measured on a live fleet config with RESEND_API_KEY unset: resend Connected, and the header the
// helper produces authenticates against the Resend API (read-only GET, HTTP 200).
//
...
const ISOLATED_CONFIG_SKIP = new Set(['settings.json', 'plugins', '.credentials.json', 'skills', 'projects'])
...
function writeJsonAtomic(path: string, value: unknown, opts: { groupShared?: boolean } = {}): void {
  ...
  // than what was on disk -- capping at the intended envelope (0o600, or 0o660
  // when group-shared) removes the only way this could preserve a pre-existing
  // world-writable (or, for a non-shared file, group-writable) mode on a
  // credential file. The group-shared OR below runs AFTER the cap so it only
  // ever ADDS the group bits this option explicitly asks for, never widens past it.
  const cap = opts.groupShared ? 0o660 : 0o600
  let mode = cap
  try { mode = statSync(path).mode & 0o777 & cap } catch { /* new file -> owner-only (or group-shared) default */ }
 ...
 * each caller.
 *
 * MERGE NOTE (B-wave, card 42938a74): adopted from upstream, but the door in
 * THIS fork is startAgentProcessUnlocked -- startAgentProcess is the
 * withLifecycleLock wrapper (card 74ba7c78) and delegates here, so every fresh
 * caller still passes exactly one place. The fork's own duplicate
 * scheduleRecoveryBrief in channel-monitor.ts is removed by the same merge, so
 * the brief is scheduled once rather than twice on the watchdog path.
 */
...
async function startAgentProcessUnlocked(name: string, opts: { fresh?: boolean } = {}): Promise<{ ok: boolean; pid?: number; error?: string }> {
  ...
  try {
    ...
    // MERGE NOTE (B-wave, card 42938a74): upstream folded the survey variable into THIS line while
    // the fork already ships it as its own `feedbackSurveyEnv` const below (card 268b257a). The
    // auto-merge left both, so the command exported the same variable twice with two comments
    ...
    // Upstream measured the same failure independently on 2026-09-11: FOUR agents frozen on this
    // dialog at once, one of them for 23 HOURS, while every external indicator read healthy
    // (running=true, messages status='delivered' -- which only means the text reached the PANE --,
    ...
    // for this file says upstream's umask 002 + agentTmuxTarget(name)/startTarget change is NOT
    // adopted because "the DEPENDENCIES are a feature... with NO runAsUser consumer in the fork".
    // That claim is stale: agentTmuxTarget(name) is now a fully-built function (defined above, this
    // file) with FOUR other non-conflicting call sites already in this file alone (getAgentRunState,
    // stopAgentProcess, killAgentSession, and this function's own later target read). The dependency
    // this rule said was missing has since been built. Adopted upstream's version instead of the
    // archived instruction, for QA/Cybersec review.
    //
