...
- **CostOps-mag port (iszzu80 fork, same-lineage superset)**: a `src/costops/*` modult (44 fájl — költség-főkönyv, collectorok, limitek/alertek, előfizetések, forecast, árazás) átemeltük az `iszzu80/develop` fork ugyanazon 011a2c4 bázisú, dúsított verziójából, izolált worktree-ben. `initCostOpsSchema` a `db.ts` init-láncába kötve, felváltva a korábbi 2-táblás stub sémát. Csak BE-mag: a meglévő élő `GET /api/costs/summary` (v0.1 read API, `costs.ts`) tovább fut a bővített modul felett; a **HTTP-route egyesítés + a valós heti-limit-% dashboard-csempe** külön, saját kártya (nem ehhez a porthoz tartozik). Új opcionális env-változók (mind grácia-fallbackosak, sosem boot-refuse): `COSTOPS_DEFAULT_OWNER`, `COSTOPS_GITHUB_BILLING_USER`, `RENDER_API_KEY` (a collectorok API-kulcsa, csak ha valaki bekapcsolja őket -- alapból nincsenek beütemezve). Modul-szintű doksi: [src/costops/README.md](src/costops/README.md).
