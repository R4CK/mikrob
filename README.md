...
- **Kvóta-menedzsment, három lépcsőben**: 5 órás session-limit figyelés + banner-detektálás (auto-resume a valós reset-időre); **heti-limit dinamikus küszöbbel** (90/92/95% a resetig hátralévő idő szerint) a teszt-stophoz; és egy enyhébb **új-fejlesztés-stop** (`newDevStopActive`, alap 60%) ami CSAK az új planned-kártya dispatchet tiltja — a gate-munka és a folyamatban lévő kártyák befejezése tovább megy, és minden idle nem-gate ügynök aktívan parkolásra kerül, hogy ne égessen kvótát üres self-advance ciklusokban.
...
- **Lokális-LLM auto-router policy-jel-család** (`src/local-llm-router.ts`): az offload-router a mechanikus-vs-online döntésnél a veszélyes authz/policy-megfogalmazásokat (access-default, tenant-scope-drop, validáció-kliensre-tolás, jogosultság-emelés) ONLINE-ra kényszeríti — a döntés a jel-CSALÁDRA általánosít, nem szó szerinti string-mintára, hedge/bizonytalan vagy üres input szintén ONLINE-ra esik (fail-closed alapállás). A local output DRAFT-only marad, kötelező MikroB+gate újra-ellenőrzéssel.
...
- **`web/app.js` / `web/lang/{hu,en}.js` / `web/style.css` -- MÉRT eredmény: JELENLEG NULLA konfliktus egy valódi merge-nél.** A fork-kód (~496 hivatkozás: local-LLM offload, ramp, fleet-park, guard-hook UI) az upstream-shared függvényeken BELÜL van interleaved (pl. a `switchPage(pageId)` router-ág), mert a `web/app.js` egyetlen 18,5k soros, bundler nélküli globális script (`<script src="/app.js">`) -- fizikai overlay-fájlba szervezés hook-keret nélkül nem lehetséges retroaktívan, és a mérés szerint nem is indokolt (upstream a merge-base óta ~40 sort változtatott, más régióban, mint a fork-kód). A `src/__tests__/fork-upstream-conflict-guard.test.ts` ezt a NULLA-konfliktus állítást ellenőrzi minden futáskor (hálózat-függő, elérhetőség hiányában dokumentáltan skip-el).
