...
- **Frontend:** van, a legtöbb API-hoz — DE a fiók-/kulcs-adminisztrációból KI VAN ZÁRVA: nem tud usereket listázni/létrehozni/törölni, nem tud device-kulcsokat listázni/létrehozni/visszavonni, és nem tud jelszót resetelni (`USER_ADMIN_KINDS`/`DEVICE_KEY_ADMIN_KINDS`, `src/web/routes/auth.ts`) — ezeken a végpontokon 403-at kap. Minden más területen (kanban, memória, vault, ügynökök) ugyanazt a hozzáférést kapja, mint a fő dashboard-token.
- **Hiányzó funkció:** finomabb device-szintű scope-szűkítés (pl. "ez a telefon csak olvashat, nem törölhet") a fiók-adminisztráción kívüli területekre jelenleg nincs — ott minden device-kulcs teljes hozzáférést ad, csak a REVOKE-álhatóság különbözteti meg a fő tokentől.
