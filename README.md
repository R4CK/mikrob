...
A lista **kategóriákba** van rendezve (11, alább), hogy egy új olvasó ne egy 95+ elemű lapos listában keressen. **Új fork-fejlesztés mindig egy meglévő kategóriába kerül** (vagy -- ha tényleg egyik sem illik -- egy új, 12. kategóriát nyit), sose a lista végére fűzve kategória nélkül.

### Flotta-orchesztráció (kanban, dispatch, gate-ek)
...
- **Kártyahivatkozás mindkét olvasónak**: egy üzenetbe vagy kommentbe írt kártya-azonosító a mentés előtt a `#sorszám (azonosító)` alakra normalizálódik. A sorszám a dashboardnak szól, a zárójeles azonosító pedig annak az ügynöknek, aki a kártyát meg is akarja nyitni: az API kizárólag azt fogadja el. Az átalakítás önmagára is alkalmazható, tehát egy idézett szöveg nem torzul tovább.
- **Helyi-LLM draft elbírálás kikényszerítése**: egy kártya, amire a helyi modell draftot posztolt, nem adható át a gate-nek, amíg valaki kimondottan el nem bírálta (elfogadva / részben / elutasítva / felesleges). Mind a négy kimenet elfogadható; a draft nem magát bírálja el, és a jelzésnek a legfrissebb draftnál újabbnak kell lennie. A „felesleges” arra való, amikor a draft jó volt, de nem vettek át belőle semmit -- elutasításként könyvelve ez a helyi modell minőségi hibájának látszana, pedig időzítés kérdése. A négy érték egyszerre mozog a guardban, a hibaüzenetben, a skillben és a dispatcher szövegében: ezt teszt tartja együtt, nem figyelem.
- **Helyi-LLM-draft egységes belépési pont önjáró kártyafelvételnél**: amikor egy szerep-ügynök a saját soros dispatchjében veszi fel a következő kártyát (nem a központi orchesztrátortól kapja), ugyanaz az egyetlen script fut le, ami a másik dispatch-útvonalon is: modell-egészség-ellenőrzés, majd a kártya tartalma alapján helyi/online döntés, LOCAL esetén helyi draft-kísérlet és a felvétel mindkét úton egységes attribúcióval naplózva. A draft-kísérlet sosem gátolja magát a kártyafelvételt, és egyetlen kapcsolóval teljesen kikapcsolható.
- **Helyi-LLM telepítettség-kapu**: hálózat nélküli, olcsó előellenőrzés, ami a teljes helyi-először draft-ágat (önjáró kártyafelvétel, dispatch-időben futó klasszifikáció, offload) egyben kihagyja azon a gépen, ahol nincs telepítve helyi modell -- külön állapot attól, hogy a telepített modell épp fut-e.
...
- **Hatókör-állítás ellenőrző a teszteken**: megnézi, hogy egy teszt-fájl fejléce állít-e olyan API-útvonalat, amit a fájl valójában soha nem ér el. A csendes üres lefedettséget célozza: a zöld futás és a fájl neve önmagában nem bizonyíték. Jelentő eszköz, nem kapu -- a forrás-kontraktus-őrök jogosan nem hívják a kiszolgálót.
- **Lint-racsni a landolási kapuban**: a lint-hibák száma szabályonként nem nőhet egy landolással; a meglévő hátralék megmarad, de minden új hiba blokkol. Ha maga a MÉRÉS romlik el -- a fa nem elemezhető, több korlátozott szabály egyszerre pontosan nullára esik, vagy a futás fájlokat néz végig és nulla leletet ad --, a racsni ezt külön mondja ki, megnevezi melyik jel szólalt meg, és nem hasonlít; ilyen állapotban a baseline frissítését is megtagadja. Korlátot létrehozni külön parancs (`--bootstrap`), hogy a baseline törlése ne kapcsolhassa ki csendben az egészet.
...
- **Projekt-prioritás a kiosztásnál**: a dashboard kanban-szűrősorában egy vagy több projekt kijelölhető; a kijelölt projektek kártyái mennek elöl minden kiosztásnál, a felsorolás sorrendjében. Üres lista esetén a sima kártya-prioritás dönt.
- **Elavult dispatch eldobása kézbesítéskor**: egy kártya-dispatch, ami sorban állt, amíg a címzett dolgozott, kézbesítés előtt újra megnézi a táblát -- ha a kártya azóta elkészült vagy gate-re került, a router eldobja a sor lezárásával (időbélyeggel és indokkal, a tartalom megmarad) ahelyett, hogy elvinne egy kört. Mérve: egy ügynök tíz sorban álló dispatchjéből hét már elavult volt, mert a munkát a tábláról vette fel. Fail-open: ha a kártya nem olvasható ki, kézbesít.
- **Üzenet-szintű ébresztési osztály (`wake`)**: egy inter-agent üzenet kimondhatja, hogy a címzett következő természetes ellenőrzésére szól, nem azonnali megszakításra. Az ilyen üzenet ugyanúgy bekerül a postafiókba és ugyanúgy kézbesül, csak nem indít miatta senki külön kört. Alapértelmezés az ébresztés, és csak a fő-ügynöknél lehet lekapcsolni, mert egyedül neki van saját, magától futó postafiók-ellenőrzése. A csendet nem szabad szöveggel kérni: a küldő egy nevesített üzenetosztályt ad meg, és csak a kódban rögzített, zárt listán szereplő osztály kap csendet -- minden más, ismeretlen vagy elgépelt osztály ébreszt.
...
- **Kártyazárás-ellenőrző**: megmondja egy kártya kommentjeiből, hogy a kijelölt gate-ek verdiktjei ugyanarra a szállítmányra szólnak-e, és hogy az a szállítmány az, amit a kártya most ad. Az összevetés tartalom-alapú, nem sha-egyenlőség: egy munka-commit és az őt landoló merge különböző shák ugyanarról a kódról. Ha egyik commit sem oldható fel, azt külön állapotként mondja ki, nem jóváhagyásként. A verdikt SZERZŐJÉT is nézi: egy olyan PASS, amit nem az a gate írt és amitől a gate maga sosem adott verdiktet, külön válasszal áll meg a csendes rendben helyett. A gate saját korábbi verdiktje veszi át a helyét, ha van; egy elutasítás viszont attól függetlenül érvényes, ki írta le. Végül megkérdezi azt is, hogy a jóváhagyott commit egyáltalán rajta van-e a fő ágon: egy tökéletesen egyetértő verdikt-sor szólhat olyan munkára, ami sosem landolt, és csak egy ágon létezik. A vizsgálat leszármazás-alapú, nem sha-egyenlőség, mert a landolás merge-dzsel történik; ha a kérdés nem megválaszolható, azt is kimondja jóváhagyás helyett.
- **Landolási gate-verdikt kapu**: a push előtt megkérdezi a táblát, hogy a landolni készült shára tényleg van-e áteresztő gate-verdikt, ahelyett hogy a landoló saját "gate-teljes" állítását elhinné. Egy áteresztő verdikt csak attól számít, aki azt a gate-szerepet viseli; egy elutasítás attól számít, aki leírta, és semmilyen kapcsoló nem írja felül.
- **Landolás-előkészítő + teljes-suite bizonyíték**: egy külön, kártyánként hívható lépés megépíti a merge-commitot, lefuttatja rá a teljes suite-ot a szemafor-szkripten át, és a végeredményt (sha, a merge FA-hash-e, teszt-számok, log-útvonal, futtató) GÉPILEG rögzíti -- nem szabad szöveges kommentből olvasva. A tényleges landolás a push előtt ezt a rekordot nézi meg a saját merge-eredménye fa-hash-éhez: egyezés esetén mehet, eltérés vagy hiány esetén off/warn/enforce kapcsoló szerint viselkedik. A teljes suite-ot maga a landoló sosem futtatja újra.
...
- **Tömeges státuszírás attribúció-kényszere**: a kanban státusz-eseményei mostantól indokot is hordoznak, és amint egy rövid időablakon belül sok kártya státusza változik, a további státuszírás csak felelőssel (`actor`) és indokkal (`reason`) megy át. Egy-egy kártya mozgatását nem érinti; a `force` nem kerüli meg, mert az más állítás. A visszautasítás nem ír eseményt, tehát az ablak magától kiürül.
...
### Git/worktree/landolási infrastruktúra
...
- **Fork-oldali horgony az ütközés-mentesítéseken**: a fork/upstream ütközés-guard mentesítései eddig csak az upstream mozgására avultak el. Egy mentesítés mostantól megnevezhet egy tényt a SAJÁT fánkról is (van-e ott egy adott szimbólum), és ha az a tény megváltozik, a mentesítés újradöntést kér. Opcionális: csak a ténylegesen megadott horgony kényszerül ki.
- **Landolás utáni automatikus újraépítés**: ha egy landolás forrást érint, a landoló a sikeres push után újraépíti az élő telepítés build-könyvtárát, sorosítva és a landolást soha meg nem buktatva. Szolgáltatást nem indít újra -- az külön, megerősített lépés marad.
...
- **Kártya-építési útvonalválasztó**: dispatch-időben eldönti, hogy egy egyszerű, jól körülhatárolt kártya első teljes draftját a helyi modell írja-e meg. Minden kétséget ONLINE-ra old fel, és a kártya ugyanúgy végigmegy a szokásos gate-eken.
- **Útvonalválasztás-audit**: összeveti, hány kártya ment ténylegesen munkába azzal, hány útvonal-döntés készült rájuk, és jelenti, ha a döntési lépés kimaradt a dispatchből. Riport, nem kapu: nem blokkol dispatchet és nem mozgat kártyát. Csak a bekapcsolása utáni dispatcheket nézi, így a régi kártyák nem termelnek hamis leletet.
- **Kétrepós kártya-bizonyíték ellenőrzés**: megnézi, hogy egy kanban-kártyát megnevez-e commit BÁRMELYIK repóban, amiben a flotta dolgozik, és külön jelöli az explicit hivatkozást a puszta említéstől. A rövidített commit-azonosítót nem téveszti össze kártya-azonosítóval. Ha az egyik repó nem olvasható, megtagadja a választ ahelyett, hogy nullát jelentene.
...
- **Modell-szintű kill switch a helyi LLM-eken**: egyetlen helyi modell kikapcsolható a dashboardról törlés nélkül (rossz kimenet vagy VRAM-hiány esetén); a letiltott modellt sem a router, sem a CLI nem használja, a lista pedig kimondottan jelzi a letiltott állapotot. A kapcsolók akkor is látszanak és működnek, ha az Ollama épp nem fut: a lista ilyenkor a bemért modellekből áll, jól látható „Az Ollama most nem fut" jelzéssel, aktiválás és letöltés nélkül.
- **Lokális LLM oldal: modellek elöl, feladat-routing láthatóan**: a dashboard Lokális LLM oldalán az Állapot után rögtön a telepített modellek listája jön (darabszám és letiltott-szám kiírva, minden soron kapcsolóval és azzal, mely presetek mennek rá), az offload-agresszivitás csúszka marad, a régi Kategóriák szekció helyén pedig a Feladat-routing szekció mutatja presetenként a be/ki kapcsolót és a hozzárendelt modellt (override vagy alapértelmezett), külön panelen a mindig online kategóriákat és az utolsó kártya-szintű helyi/online döntéseket. Az Áttekintés modell-elosztás swimlane-jén a letiltott modell jelvényt kap, a sáv címkéje a Lokális LLM oldal megfelelő sorára visz, a buborék pedig megmondja, miért az a modell kapta a feladatot.
- **VRAM-kapu a helyi-LLM dispatch előtt**: megnézi a GPU tényleges memória-terhelését, és magas terhelésnél nem enged új helyi dispatchet indulni, hanem online-ra terel. A terhelést hiszterézissel méri, tehát egyetlen zajos mintavétel nem billenti át. Ha a mérőeszköz nincs is a gépen, átenged (nincs mit védeni), ha viszont ott van és nem válaszol, visszatart -- kétség esetén online. A terhelést ATTRIBUÁLJA is: a saját, betöltött modellünk memóriáját levonja, és a maradékra, az idegen terhelésre dönt, mert egy puszta százalék nem tudja megkülönböztetni a saját meleg modellünket egy idegen GPU-fogyasztótól. Amíg a saját munkánk tartja a GPU-zárat, a mérés nem is számít idegen terhelésnek.
- **Megosztott instrukció-fájl mint önálló ONLINE-ok**: ha egy kártya terméke olyan fájl, amit utána minden ügynök végrehajt (skill, ügynök-prompt, ütemezett feladat leírása, hook, beállítás-fájl), a helyi-modell útja zárva marad, függetlenül attól, mennyire egyszerűnek látszik a munka. Az indok az, hogy ott a termék próza: egy rossz mondattól semmi nem lesz piros, tehát nincs mögötte teszt, ami elkapná.
...
- **Draft-értesítés a kártya felelősének**: amikor a helyi modell draftja felkerül egy kártyára -- vagy kiderül, hogy draft nem lesz --, a kártya felelőse azonnal kap egy jelzést, hogy még munka közben tudjon róla, ne a végén. Ha a felelős éppen parkolva van, vagy a kártyának nincs felelőse, MikroB kapja, így az értesítés nem vész el csendben.
...
- **Cybersecurity-Skills Tier-1 vendorolt per-ügynök skillek**: 14 külső biztonsági skill (API-jogosultság, OAuth/JWT, CORS és host-header, SSRF, rate-limit, prompt-injection-detektálás, supply-chain, e-mail-hitelesítés) csak a két biztonsági ügynökhöz vendorolva, rögzített upstream commiton, használati korlátozással; a többi ügynök kontextusát nem terheli.
- **Élő skill-másolat drift-szinkron, változás-alapú riasztással**: egy már futó ügynök saját `.claude/skills/` másolatát semmi nem frissítette egy seed-javítás után, tehát a javítás után is a régi utasításokat olvasta. Egy hatóránként futó eszköz ezt zárja: kizárólag a bizonyíthatóan csak-elavult (egy korábban kiadott verzióval byte-azonos) másolatot szinkronizálja, a kézzel patcheltet soha, és TOCTOU-őrrel átugorja azt, amit közben más írt. Egy FUTÓ ügynök fájába egyáltalán nem ír: a futás tényét a gépen futó tmux-munkamenetekből olvassa, és amit nem tud bizonyítottan parkoltnak látni, azt futónak tekinti -- a kihagyott másolat a következő körben szinkronizál, és a verdikt-sor megnevezi, hány maradt így hátra. A riasztás a diverged **halmaz változására** szól, nem a darabszámára -- egy állandó, jogos eltérés-halmaz különben hatóránként ugyanazt az üzenetet küldené, amit két hét után senki nem olvas.
...
- **Dashboard API-kivonatoló**: kis helyi szkript, ami a kanban/agent/üzenetsor API-válaszait olvashatóra sűríti (kártya, kommentek, oszlop, ügynökök, függő üzenetek). A tokent maga kezeli, sosem írja ki, és minden rövidítést `...(+N)` jelöl, hogy a hiányzó rész mérete látszódjon.
- **Suite-szemafor**: a teljes mopsion tesztfutásokat flottaszinten kettőre korlátozza, a többi sorba áll. A gép 12 magján három egyidejű futás CPU-éheztetést okoz, amitől a vitest worker-RPC-je időtúllépésbe fut, és a futás nulla teszt-bukás mellett bukik el. A várakozó futás nem néma: `PAUSED-SEMAPHORE`/`RESUMED-SEMAPHORE` jelzést ír a kártyájára, különben a beragadás-figyelő elvenné tőle a munkát. A slot mellé memória-előfeltétel is tartozik: egy futás csak akkor indul el, ha a gépen van elég szabad memória hozzá, különben sorba áll ugyanúgy, mint slothiánynál -- a visszautasítás megnevezi, melyik a kettő közül.
- **Vitest-flake osztályozó**: egy befejezett teszt-futás naplójából megmondja, hogy a nemnulla kilépés valódi hiba-e, vagy a vitest saját, konfigurálhatatlan worker-RPC időtúllépése, amitől a futás nulla bukott teszttel is 1-gyel tér vissza. A kilépési kódot SOHA nem írja át, csak megnevezi, mit lát -- és csak akkor, ha a naplóban egyetlen bukott teszt sincs.
- **Kihagyott-teszt jelentés a futás végén**: egy befejezett teszt-futás után kimondja, hány tesztfájl és hány teszt nem futott le egyáltalán, és ebből mennyi az, ami a beállítatlan adatbázis-környezetváltozóra hivatkozik a kódjában; amit nem tud besorolni, azt külön nevezi meg, nem olvasztja bele. A kilépési kódot nem írja át, és hallgat, ha semmi nem maradt ki. A cél az, hogy egy zöld futás ne olvasódjon bizonyítéknak olyan tesztekről, amelyek el sem indultak.
- **Gépszintű suite-zár a marveen tesztfutásokon**: egyszerre csak egy teljes tesztfutás mehet ezen a gépen, sorba állítva, korlátos várakozással. A privát tesztfa nem kilépő a sorból, csak a futás helyét választja meg, és a feloldott zár-útvonal lekérdezhető a szkripttől.
- **Selftest-felfedezés**: minden `store/*.selftest.sh` és `store/*.selftest.py` automatikusan lefut a suite-ban, felfedezés alapján -- nem kézzel írt wrapperenként. Egy újonnan hozzáadott selftest a landolása pillanatában be van kötve, nincs mit elfelejteni. A ritka kivételt indoklás nevezi meg, és egy teszt kiköti, hogy a kivétel-bejegyzés célja még létezzen. Enélkül a megírt, zöldnek látszó ellenőrzések évekig futhatnak nulla alkalommal.
...
- **GPU crash-loop őr**: egy percenkénti figyelő, ami a boot-hosszakból és a kernel-hibaszövegből felismeri a GPU-passthrough okozta WSL-újraindulási hurkot, leállítja és maszkolja az érintett unitot, és riasztja a tulajdonost. A maszkolást a systemd állapotából ellenőrzi (nem a parancs kilépőkódjából), és ha egy valódi unit-fájl útban van, a kézi utat futtatja le. Az állapotjelzőjét minden körben visszaméri, tehát egy kézi visszakapcsolás után magától eltűnik. A `--restore-hint` kiírja a teljes visszaállító parancsot, mert a puszta `unmask` nem elég.
- **Engedélykérés-felismerés a menü-helyreállítóban**: a session-figyelő megkülönbözteti a beragadt interaktív menüt a Claude Code tool-engedélykérésétől. Menünél továbbra is Escape-et küld, engedélykérésnél viszont egyetlen billentyűt sem nyom -- ott az Escape NEM-et jelentene az operátor nevében --, hanem riaszt, hogy ember döntsön.
- **Token-használat dedup-takarító**: egyszeri, visszaforditható tisztító a `token_usage` tábla kereszt-ügynök duplikátumaira. Alapból csak olvas, és három túlélő-szabályt mér össze ugyanazon az adaton (legkisebb id, legtöbb kitöltött mező, attribúció-biztos), hogy a törlés előtt látszódjon, melyik mit dobna el. Az éles futás minden érintett sort kiment egy visszaállító fájlba, mielőtt bármit törölne.
...
Kártya `641aca3f` (Peti weekly-stop kivétel, MikroB-dispatch), újramérve `eba65f46`-ban. A cél: egy jövőbeli `git merge upstream/develop` **NULLA konfliktust** adjon az upstream-owned fájlokon. Mielőtt bármit mozgatnánk, egy valódi `git merge --no-commit --no-ff upstream/develop` dry-run-nal (eldobható worktree) MÉRVE lett a tényleges konfliktus-felület — lásd a `src/__tests__/fork-upstream-conflict-guard.test.ts` guard-tesztet, ami ugyanezt a mérést automatizálja -- a HÁLÓZATFÜGGŐ fele (élő `upstream` fetch + valódi merge dry-run) 2026-09-06 óta nem a landolási kapuban fut, hanem a `fork-upstream-drift-watch` napi ütemezett feladatban (kártyák `99c2eb09`, `5da60b85`, `a1ce8952`); a szerkezeti állítások változatlanul minden landoláskor futnak. **A cél 2026-08-14 és 2026-08-17 között teljesült** a `web/` fájlokra (kártya `241532d8`): a guard ZÖLD volt, valódi, ARMED merge dry-runnal. **2026-08-17-én ÚJRA MEGDŐLT (kártya `b91f11d8`)** -- lásd a `web/app.js` bekezdés végén a friss mérést. A guard ugyanígy pirosra vált, ha bárki visszaír fork-kódot egy megosztott upstream-függvénybe.
